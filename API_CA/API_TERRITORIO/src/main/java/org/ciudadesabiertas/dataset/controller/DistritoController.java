...
import org.ciudadesabiertas.dataset.utils.DistritoResult;
import org.ciudadesabiertas.dataset.utils.DistritoSearch;
import org.ciudadesabiertas.dataset.utils.DistritoSearchQuery;
...
@RestController
@Api(value="Territorio - Distrito",description = "Conjunto de operaciones relacionadas con el conjunto de datos Sección Censal"+SwaggerConstants.VOCABULARIO_A_HREF+TerritorioConstants.distritoVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Territorio - Distrito"})
public class DistritoController extends GenericController implements CiudadesAbiertasController 
{
	

	public static final String LIST = "/territorio/distrito";
	...
	public static final String MODEL_VIEW_LIST = "territorio/distrito/list";
	
	public static final String MODEL_VIEW_ID = "territorio/distrito/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = DistritoController.class.getName();
	
	private static final String NAME_FIELD_GEOJSON = "CUDIS";
	...
	private static final Logger log = LoggerFactory.getLogger(DistritoController.class);
		

	@Autowired
	protected DatasetService<Distrito> service;
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new Distrito(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,DistritoSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=DistritoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, DistritoSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<Distrito>, EntityManager> visitor = new JpaCriteriaQueryVisitor<Distrito>();
		
		ResponseEntity list= listDistrito(request, search, fields, rsqlQ, page, pageSize, sort, srId, LIST,new Distrito(), new DistritoResult(), 
		...
		if (statusCode.is2xxSuccessful())
		{
			...
			Result<Distrito> result=((Result<Distrito>)body);
			
			List<Distrito> records = result.getRecords();
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, DistritoSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=DistritoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required=true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_DISTRITO) String id,
			@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
		...
		ResponseEntity record = record(request, id, new Distrito(), new DistritoResult(),srId, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required=true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_DISTRITO) String id,
			@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
	...
	public @ResponseBody ResponseEntity<Object> geometry(HttpServletRequest request, 
													@PathVariable @ApiParam(required=true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_DISTRITO) String id,
													@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
		...
		if (statusCode.is2xxSuccessful())
		{
			...
			if (isSemantic)
			{
				...
				g.setId(distrito.getTitle());
				g.setTypeForURI("Distrito");
				g.setGeometry(distrito.getHasGeometry());
				listado.add(g);
			}
			else
			{
				listado.add(distrito.getHasGeometry());
			}									
			
			resultObject.setRecords((List<Object>) listado);
			
			//MD5
			resultObject.setContentMD5(Util.generateHash( distrito.getHasGeometry().toString() ));
	 ...
	 */
	@SuppressWarnings("unchecked")
	private  ResponseEntity<?> listDistrito(HttpServletRequest request, DistritoSearch search, 
			 String fields, 
			 String rsqlQ, 
			 String page, 
			 String pageSize,
			 ...
			 String operacion,
			 Distrito objModel,
			 DistritoResult objResult,
			 List<String> availableFields,
			 String key,
			 RSQLVisitor<CriteriaQuery<Distrito>, EntityManager> visitor,
			 DatasetService<Distrito> dsService)
	{
		...
		//Consulta RSQL			
			try
			{
				...
				Result<Distrito> result =dsService.searchByRSQLQuery(visitor,key, rsqlQ, numPage, numPageSize, orders);				
		...
		//Consulta estandar
			...
			try
			{	
				DistritoSearchQuery searchQuery = (new DistritoSearchQuery()).toTransform(search);
				...
				if (search.getMunicipioId()!=null) {
					...
					if (objMunicipio!=null) {
						...
						return responseEntity = guardarResult(srId, new ArrayList<Distrito>(), 0, numPageSize, objResult, request)	;
					}					
				}
				...
				listado = dsService.query(key,(Class<Distrito>) objModel.getClass(),(DatasetSearch<Distrito>) searchQuery, numPage, numPageSize,fieldsQuery, orders);														
				
				long total=dsService.rowcount(key,(Class<Distrito>) objModel.getClass(),(DatasetSearch<Distrito>) searchQuery);				
