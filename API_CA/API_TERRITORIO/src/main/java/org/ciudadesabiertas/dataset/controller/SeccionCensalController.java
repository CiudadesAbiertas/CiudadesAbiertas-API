...
import org.ciudadesabiertas.dataset.utils.SeccionCensalResult;
import org.ciudadesabiertas.dataset.utils.SeccionCensalSearch;
import org.ciudadesabiertas.dataset.utils.SeccionCensalSearchQuery;
...
public class SeccionCensalController extends GenericController implements CiudadesAbiertasController 
{
	...
	public static final String MODEL_VIEW_LIST = "territorio/seccion/list";
	public static final String MODEL_VIEW_ID = "territorio/seccion/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = SeccionCensalController.class.getName();
	
	private static final String NAME_FIELD_GEOJSON = "CUSEC";
	...
	private static final Logger log = LoggerFactory.getLogger(SeccionCensalController.class);
		

	@Autowired
	protected DatasetService<SeccionCensal> service;
				...
				@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new SeccionCensal(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,SeccionCensalSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=SeccionCensalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<SeccionCensal>, EntityManager> visitor = new JpaCriteriaQueryVisitor<SeccionCensal>();
		
		ResponseEntity list= listSeccionCensal(request, search, fields, rsqlQ, page, pageSize, sort, srId, LIST,new SeccionCensal(), new SeccionCensalResult(), 
		...
		if (statusCode.is2xxSuccessful())
		{
			...
			Result<SeccionCensal> result=((Result<SeccionCensal>)body);
			
			List<SeccionCensal> records = result.getRecords();
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=SeccionCensalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
		...
		ResponseEntity record = record(request, id, new SeccionCensal(), new SeccionCensalResult(), srId, nameController, RECORD, service,getKey());
													...
													@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
		...
		if (statusCode.is2xxSuccessful())
		{
			...
			if (isSemantic)
			{
				...
				g.setId(sCensal.getTitle());
				g.setTypeForURI("Barrio");
				g.setGeometry(sCensal.getHasGeometry());
				listado.add(g);
			}
			else
			{
				listado.add(sCensal.getHasGeometry());
			}									
			
			resultObject.setRecords((List<Object>) listado);
			
			//MD5
			resultObject.setContentMD5(Util.generateHash( sCensal.getHasGeometry().toString() ));
...
/** METODOS PRIVADOS **/
	 ...
	 */
	@SuppressWarnings("unchecked")
	private  ResponseEntity<?> listSeccionCensal(HttpServletRequest request, SeccionCensalSearch search, 
			 String fields, 
			 String rsqlQ, 
			 String page, 
			 String pageSize,
			 ...
			 String srId,	
			 String operacion,
			 SeccionCensal objModel,
			 SeccionCensalResult objResult,
			 List<String> availableFields,
			 String key,
			 RSQLVisitor<CriteriaQuery<SeccionCensal>, EntityManager> visitor,
			 DatasetService<SeccionCensal> dsService)
	{
		...
		//Consulta RSQL			
			try
			{
				...
				Result<SeccionCensal> result =dsService.searchByRSQLQuery(visitor,key, rsqlQ, numPage, numPageSize, orders);				
		...
		//Consulta estandar
			List<SeccionCensal> listado;
			try
			{	
				SeccionCensalSearchQuery searchQuery = (new SeccionCensalSearchQuery()).toTransform(search);
