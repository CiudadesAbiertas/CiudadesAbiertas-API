...
@RestController
@Api(value="Territorio - Autonomía",description = "Conjunto de operaciones relacionadas con el conjunto de datos Autonomía"+SwaggerConstants.VOCABULARIO_A_HREF+TerritorioConstants.autonomiaVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Territorio - Autonomía"})
public class AutonomiaController extends GenericController implements CiudadesAbiertasController 
{
	

	public static final String LIST = "/territorio/autonomia";
	...
	private static String nameController = AutonomiaController.class.getName();

	private static final String NAME_FIELD_GEOJSON = "CCA_2";
	...
	private static final Logger log = LoggerFactory.getLogger(AutonomiaController.class);
		

	@Autowired
	protected DatasetService<Autonomia> service;
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new Autonomia(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,AutonomiaSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=AutonomiaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, AutonomiaSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<Autonomia>, EntityManager> visitor = new JpaCriteriaQueryVisitor<Autonomia>();
			
		ResponseEntity list= listAutonomia(request, search, fields, rsqlQ, page, pageSize, sort, srId, LIST,new Autonomia(), new AutonomiaResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, AutonomiaSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=AutonomiaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required=true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_AUTONOMIA) String id,
			@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
		...
		//return record(request, id, new Autonomia(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required=true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_AUTONOMIA) String id,
			@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
	 ...
	 */
	@SuppressWarnings("unchecked")
	private  ResponseEntity<?> listAutonomia(HttpServletRequest request, AutonomiaSearch search, 
			 String fields, 
			 String rsqlQ, 
			 String page, 
			 String pageSize,
			 ...
			 String operacion,
			 Autonomia objModel,
			 AutonomiaResult objResult,
			 List<String> availableFields,
			 String key,
			 RSQLVisitor<CriteriaQuery<Autonomia>, EntityManager> visitor,
			 DatasetService<Autonomia> dsService)
	{
		...
		//Consulta RSQL			
			try
			{
				...
				Result<Autonomia> result =dsService.searchByRSQLQuery(visitor,key, rsqlQ, numPage, numPageSize, orders);				
		...
		//Consulta estandar
			List<Autonomia> listado;
			try
			{
				AutonomiaSearchQuery searchQuery = (new AutonomiaSearchQuery()).toTransform(search);
				...
				listado = dsService.query(key,(Class<Autonomia>) objModel.getClass(),(DatasetSearch<Autonomia>) searchQuery, numPage, numPageSize,fieldsQuery, orders);														
