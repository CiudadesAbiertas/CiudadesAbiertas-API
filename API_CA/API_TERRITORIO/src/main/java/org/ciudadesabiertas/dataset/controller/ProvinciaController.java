...
@RestController
@Api(value="Territorio - Provincia",description = "Conjunto de operaciones relacionadas con el conjunto de datos Sección Censal"+SwaggerConstants.VOCABULARIO_A_HREF+TerritorioConstants.provinciaVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Territorio - Provincia"})
public class ProvinciaController extends GenericController implements CiudadesAbiertasController 
{
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		if (statusCode.is2xxSuccessful())
		{
			...
			for (Provincia provincia:records) {	
			  TerritorioUtil.addPolygon(isSemantic, (IGeoModelGeometry)provincia);
			}
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required=true,value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PROVINCIA) String id,
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required=true,value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PROVINCIA) String id,
			@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
	...
	public @ResponseBody ResponseEntity<Object> geometry(HttpServletRequest request, 
													@PathVariable @ApiParam(required=true,value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PROVINCIA) String id,
													@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
