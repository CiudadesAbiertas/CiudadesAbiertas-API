...
import org.ciudadesabiertas.dataset.utils.MunicipioResult;
import org.ciudadesabiertas.dataset.utils.MunicipioSearch;
import org.ciudadesabiertas.dataset.utils.MunicipioSearchQuery;
...
@RestController
@Api(value="Territorio - Municipio",description = "Conjunto de operaciones relacionadas con el conjunto de datos Sección Censal"+SwaggerConstants.VOCABULARIO_A_HREF+TerritorioConstants.municipioVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Territorio - Municipio"})
public class MunicipioController extends GenericController implements CiudadesAbiertasController 
{
	

	public static final String LIST = "/territorio/municipio";
	...
	public static final String MODEL_VIEW_LIST = "territorio/municipio/list";
	
	public static final String MODEL_VIEW_ID = "territorio/municipio/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = MunicipioController.class.getName();
	
	private static final String NAME_FIELD_GEOJSON = "CUMUN";
	...
	@Autowired
	protected DatasetService<Municipio> service;
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)	
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new Municipio(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,MunicipioSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=MunicipioResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, MunicipioSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<Municipio>, EntityManager> visitor = new JpaCriteriaQueryVisitor<Municipio>();
		
		ResponseEntity list= listMunicipio(request, search, fields, rsqlQ, page, pageSize, sort, srId, LIST,new Municipio(), new MunicipioResult(), 
		...
		if (statusCode.is2xxSuccessful())
		{
			...
			Result<Municipio> result=((Result<Municipio>)body);
			
			List<Municipio> records = result.getRecords();
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, MunicipioSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS)  String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=MunicipioResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required=true,value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_MUNICIPIO) String id,
			@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
		...
		ResponseEntity record = record(request, id, new Municipio(), new MunicipioResult(),srId, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required=true,value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_MUNICIPIO) String id,
			@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
	...
	public @ResponseBody ResponseEntity<Object> geometry(HttpServletRequest request, 
													@PathVariable @ApiParam(required=true,value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_MUNICIPIO) String id,
													@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value = SwaggerConstants.PARAM_SRID, allowableValues = Constants.SUPPORTED_SRIDS) String srId)
	{
		...
		if (statusCode.is2xxSuccessful())
		{
			...
			if (isSemantic)
			{
				...
				g.setId(municipio.getTitle());
				g.setTypeForURI("Municipio");
				g.setGeometry(municipio.getHasGeometry());
				listado.add(g);
			}
			else
			{
				listado.add(municipio.getHasGeometry());
			}									
			
			resultObject.setRecords((List<Object>) listado);
			
			//MD5
			resultObject.setContentMD5(Util.generateHash( municipio.getHasGeometry().toString() ));
...
/** METODOS PRIVADOS **/
	 ...
	 */
	@SuppressWarnings("unchecked")
	private  ResponseEntity<?> listMunicipio(HttpServletRequest request, MunicipioSearch search, 
			 String fields, 
			 String rsqlQ, 
			 String page, 
			 String pageSize,
			 ...
			 String operacion,
			 Municipio objModel,
			 MunicipioResult objResult,
			 List<String> availableFields,
			 String key,
			 RSQLVisitor<CriteriaQuery<Municipio>, EntityManager> visitor,
			 DatasetService<Municipio> dsService)
	{
		...
		//Consulta RSQL			
			try
			{
				...
				Result<Municipio> result =dsService.searchByRSQLQuery(visitor,key, rsqlQ, numPage, numPageSize, orders);				
		...
		//Consulta estandar
			List<Municipio> listado;
			try
			{
				MunicipioSearchQuery searchQuery = (new MunicipioSearchQuery()).toTransform(search);
				...
				listado = dsService.query(key,(Class<Municipio>) objModel.getClass(),(DatasetSearch<Municipio>) searchQuery, numPage, numPageSize,fieldsQuery, orders);														
