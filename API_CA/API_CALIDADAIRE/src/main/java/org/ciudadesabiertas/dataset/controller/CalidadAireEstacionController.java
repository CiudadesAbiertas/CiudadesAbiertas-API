...
import org.ciudadesabiertas.dataset.util.CalidadAireEstacionGeoSearch;
import org.ciudadesabiertas.dataset.util.CalidadAireEstacionResult;
import org.ciudadesabiertas.dataset.util.CalidadAireEstacionSearch;
...
@RestController
@Api(value="CalidadAireEstacion",description = "Conjunto de operaciones sobre estaciones del conjunto de datos Calidad del Aire"+SwaggerConstants.VOCABULARIO_A_HREF+CalidadAireConstants.airQualityStationVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Calidad del Aire - Estación"})
public class CalidadAireEstacionController extends GenericController implements CiudadesAbiertasController
{	
	public static final String LIST = "/calidad-aire/estacion";
	
	public static final String GEO_LIST = LIST+ "/geo";
	
	public static final String SEARCH_DISTINCT = LIST+"/distinct";
	
	public static final String RECORD = "/calidad-aire/estacion/{id}";
	...
	public static final String MODEL_VIEW_LIST = "calidadAire/estacionList";
	public static final String MODEL_VIEW_ID = "calidadAire/estacionId";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("ESTACION_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("ESTACION_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("ESTACION_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("ESTACION_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("ESTACION_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("ESTACION_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(CalidadAireEstacion.class);

	private static final Logger log = LoggerFactory.getLogger(CalidadAireEstacionController.class);
		
	@Autowired
	protected DatasetService<CalidadAireEstacion> dsService;
	
	@Autowired
	protected DatasetService<CalidadAireSensor> calidadAireSensorService;
	
	@Autowired
	protected DatasetService<CalidadAireObservacion> calidadAireObservacionService;
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new CalidadAireEstacion(), new ObjectResult(),  dsService);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request, CalidadAireEstacionSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=CalidadAireEstacionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {GEO_LIST,  VERSION_1+GEO_LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> geoList(HttpServletRequest request,	CalidadAireEstacionGeoSearch search, 
	...
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, CalidadAireEstacionSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, CalidadAireEstacionSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=CalidadAireEstacionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_CALIDADAIREESTACION_TEXT) 			
			@RequestBody CalidadAireEstacion obj 
			)
	{
		...
		return add(obj, CalidadAireEstacionController.class.getName(), ADD, dsService,getKey());
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=CalidadAireEstacionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_TEXT+SwaggerConstants.PARAM_ID_CALIDAD_AIRE_ESTACION) 
			...
			@RequestBody CalidadAireEstacion obj)
	{
		...
		return update(id, obj, CalidadAireEstacionController.class.getName(), UPDATE, dsService,getKey());
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=CalidadAireEstacionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		if (Util.validValue(id)) {
			...
			try {
				CalidadAireEstacion item= dsService.findById(getKey(),CalidadAireEstacion.class,id);
				...
				if (item==null) {
					...
					errorApi.setClassName(CalidadAireEstacionController.class.getName());
					...
					if ((numSensors==0)&&(numObservations==0))
					{
						...
						if (numSensors>0)
						{
							errorDescription="this Estacion is used "+numSensors+" times in Sensors";
						}
						if (numObservations>0)
						{
							...
							errorDescription+="this Estacion is used "+numObservations+" times in Observations";
						}
						errorApi.setError(errorDescription);
					}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=CalidadAireEstacionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_CALIDAD_AIRE_ESTACION) String id,@RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value=SwaggerConstants.PARAM_SRID, allowableValues=Constants.SUPPORTED_SRIDS)  String srId)
	{
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_CALIDAD_AIRE_ESTACION) String id, @RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value=SwaggerConstants.PARAM_SRID, allowableValues=Constants.SUPPORTED_SRIDS)  String srId)
	{
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_CALIDADAIREESTACION_TEXT) @RequestBody CalidadAireEstacion obj) {

		log.info("[transform]");

		log.debug("[parmam][obj:" + obj + "]");

		return transform(obj, CalidadAireEstacionController.class.getName(), TRANSFORM);
