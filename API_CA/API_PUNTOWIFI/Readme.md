...
# API CIUDADES ABIERTAS - MÓDULO PUNTOS WIFI
...
Para realizar esto lo único que se necesita es crear un fichero **puntoWifi.properties** e incluirlo en la carpeta de recursos del proyecto WEB (API_WEB/src/main/resources/) donde se distribuyen el resto de ficheros de configuración.

Esto permite que el módulo de **puntos wifi** utilice la información que contenga este fichero cuando se ejecutan peticiones sobre dicho módulo.

A continuación se detallan los parámetros que se pueden configurar en este fichero **puntoWifi.properties**.
...
- **EQPW_LIST**: Listado de puntos wifis
- **EQPW_RECORD**: Ficha de un punto wifi
- **EQPW_ADD**:  Alta de de un punto wifi
- **EQPW_UPDATE**: Modificación de un punto wifi
- **EQPW_DELETE**: Baja de de un punto wifi
- **EQPW_TRANSFORM**: Transformador de un punto wifi externo (a través de JSON)

Y los parámetros configurados quedarían de la siguiente manera en el fichero (**puntoWifi.properties**) Como Ejemplo:
- **peticiones.identificadas.public_auth=EQPW_ADD,EQPW_RECORD,EQPW_LIST**
- **peticiones.identificadas.basic_auth= EQPW_UPDATE,EQPW_TRANSFORM**
- **peticiones.identificadas.admin_auth= EQPW_DELETE**
