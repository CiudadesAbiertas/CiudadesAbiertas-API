...
import org.ciudadesabiertas.dataset.utils.BicicletaPublicaPuntoPasoGeoSearch;
import org.ciudadesabiertas.dataset.utils.BicicletaPublicaPuntoPasoResult;
...
@RestController
@Api(value="Punto de Paso",description = "Conjunto de operaciones relacionadas con el conjunto de datos Bicicleta pública", tags= {"Bicicleta pública - Punto de Paso"})
public class BicicletaPublicaPuntoPasoController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/bicicleta-publica/punto-paso";
	
	public static final String GEO_LIST = LIST+ "/geo";
	
	public static final String SEARCH_DISTINCT = LIST+"/distinct";
	
	public static final String RECORD = "/bicicleta-publica/punto-paso/{id}";
	...
	public static final String MODEL_VIEW_LIST = "/bicicletapublica/puntoPaso/list";
	public static final String MODEL_VIEW_ID = "/bicicletapublica/puntoPaso/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = BicicletaPublicaPuntoPasoController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("BICICLETA_PUNTO_PASO_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("BICICLETA_PUNTO_PASO_ESTACION_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("BICICLETA_PUNTO_PASO_ESTACION_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("BICICLETA_PUNTO_PASO_ESTACION_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("BICICLETA_PUNTO_PASO_ESTACION_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("BICICLETA_PUNTO_PASO_ESTACION_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(BicicletaPublicaPuntoPaso.class);

	private static final Logger log = LoggerFactory.getLogger(BicicletaPublicaPuntoPasoController.class);
		

	@Autowired
	protected DatasetService<BicicletaPublicaPuntoPaso> service;
	@Autowired
															...
															@RequestParam(value = Constants.PAGESIZE, defaultValue = Constants.defaultGroupByPageSize+"", required = false) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new BicicletaPublicaPuntoPaso(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,BicicletaPublicaPuntoPasoSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=BicicletaPublicaPuntoPasoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {GEO_LIST,  VERSION_1+GEO_LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> geoList(HttpServletRequest request,	BicicletaPublicaPuntoPasoGeoSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		ResponseEntity list= geoList(request, search, fields, meters, page, pageSize, sort, LIST, new BicicletaPublicaPuntoPaso(), new BicicletaPublicaPuntoPasoResult(), availableFields, getKey(),service);
	...
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, BicicletaPublicaPuntoPasoSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) String rsqlQ, 
			@RequestParam(value = Constants.PAGE, defaultValue = "1", required = false) String page, 
			@RequestParam(value = Constants.PAGESIZE, defaultValue = "", required = false) String pageSize,
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new BicicletaPublicaPuntoPaso(), new BicicletaPublicaPuntoPasoResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, BicicletaPublicaPuntoPasoSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) String rsqlQ, 
			@RequestParam(value = Constants.PAGE, defaultValue = "1", required = false) String page, 
			@RequestParam(value = Constants.PAGESIZE, defaultValue = "", required = false) String pageSize,
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=BicicletaPublicaPuntoPasoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_BICICLETAPUBLICAESTACION_TEXT) 			
			@RequestBody BicicletaPublicaPuntoPaso obj 
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getTrayectoId(), Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=BicicletaPublicaPuntoPasoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=BicicletaPublicaPuntoPasoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new BicicletaPublicaPuntoPaso(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=BicicletaPublicaPuntoPasoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable String id)
	{
		...
		return record(request, id, new BicicletaPublicaPuntoPaso(),new BicicletaPublicaPuntoPasoResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_BICICLETAPUBLICAESTACION_TEXT) @RequestBody BicicletaPublicaPuntoPaso obj) {
	...
	private List<String> checkClavesExternas(String trayectoId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_ADD)||(operation.equals(Constants.BASIC_OPERATION_UPDATE)))
			{
				if (trayectoId!=null)
				{
					BicicletaPublicaTrayectoSearch trayectoSearch=new BicicletaPublicaTrayectoSearch();
					trayectoSearch.setId(trayectoId);
					long rowcount = trayectoService.rowcount(getKey(), BicicletaPublicaTrayecto.class, trayectoSearch);
					if (rowcount==0)			
					{
						errors.add("El trayecto '"+trayectoId+"' no existe");		
					}
				}
