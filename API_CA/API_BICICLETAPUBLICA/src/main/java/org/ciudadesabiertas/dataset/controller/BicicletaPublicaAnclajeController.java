...
import org.ciudadesabiertas.dataset.utils.BicicletaPublicaAnclajeResult;
...
@RestController
@Api(value="Anclaje",description = "Conjunto de operaciones relacionadas con el conjunto de datos Bicicleta pública"+SwaggerConstants.VOCABULARIO_A_HREF+BicicletaPublicaConstants.bicicletaVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Bicicleta pública - Anclaje"})
public class BicicletaPublicaAnclajeController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/bicicleta-publica/anclaje";
	
	public static final String GEO_LIST = LIST+ "/geo";
	
	public static final String SEARCH_DISTINCT = LIST+"/distinct";
	
	public static final String RECORD = "/bicicleta-publica/anclaje/{id}";
	...
	public static final String MODEL_VIEW_LIST = "/bicicletapublica/anclaje/list";
	public static final String MODEL_VIEW_ID = "/bicicletapublica/anclaje/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = BicicletaPublicaAnclajeController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("BICICLETA_ANCLAJE_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("BICICLETA_ANCLAJE_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("BICICLETA_ANCLAJE_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("BICICLETA_ANCLAJE_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("BICICLETA_ANCLAJE_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("BICICLETA_ANCLAJE_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(BicicletaPublicaAnclaje.class);

	private static final Logger log = LoggerFactory.getLogger(BicicletaPublicaAnclajeController.class);
		

	@Autowired
	protected DatasetService<BicicletaPublicaAnclaje> service;
	@Autowired
	protected DatasetService<BicicletaPublicaEstacion> estacionService;
	@Autowired
	protected DatasetService<BicicletaPublicaTrayecto> trayectoService;
															...
															@RequestParam(value = Constants.PAGESIZE, defaultValue = Constants.defaultGroupByPageSize+"", required = false) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new BicicletaPublicaAnclaje(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,BicicletaPublicaAnclajeSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=BicicletaPublicaAnclajeResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, BicicletaPublicaAnclajeSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<BicicletaPublicaAnclaje>, EntityManager> visitor = new JpaCriteriaQueryVisitor<BicicletaPublicaAnclaje>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new BicicletaPublicaAnclaje(), new BicicletaPublicaAnclajeResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, BicicletaPublicaAnclajeSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) String rsqlQ, 
			@RequestParam(value = Constants.PAGE, defaultValue = "1", required = false) String page, 
			@RequestParam(value = Constants.PAGESIZE, defaultValue = "", required = false) String pageSize,
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=BicicletaPublicaAnclajeResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_BICICLETAPUBLICAANCLAJE_TEXT) 			
			@RequestBody BicicletaPublicaAnclaje obj 
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getEstacionBicicletaId(), null, Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=BicicletaPublicaAnclajeResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=BicicletaPublicaAnclajeResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new BicicletaPublicaAnclaje(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=BicicletaPublicaAnclajeResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable String id)
	{
		...
		return record(request, id, new BicicletaPublicaAnclaje(),new BicicletaPublicaAnclajeResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_BICICLETAPUBLICAANCLAJE_TEXT) @RequestBody BicicletaPublicaAnclaje obj) {
	...
	private List<String> checkClavesExternas(String estacionBicicletaId, String anclajeId, String operation) {
		...
		{	
			...
			}else if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				if (anclajeId!=null)
				{
					BicicletaPublicaTrayectoSearch search=new BicicletaPublicaTrayectoSearch();
					search.setAnclajeDestinoId(anclajeId);
					long rowcountDest = trayectoService.rowcount(getKey(), BicicletaPublicaTrayecto.class, search);
					
					search=new BicicletaPublicaTrayectoSearch();
					search.setAnclajeOrigenId(anclajeId);
					long rowcountOri = trayectoService.rowcount(getKey(), BicicletaPublicaTrayecto.class, search);
					
					if (rowcountDest>0 || rowcountOri>0)			
					{
						errors.add("El anclaje '"+anclajeId+"' es usado en relación de trayecto");
					}
