...
import org.ciudadesabiertas.dataset.util.CallejeroTramoViaSearch;
import org.ciudadesabiertas.dataset.util.CallejeroViaResult;
import org.ciudadesabiertas.dataset.util.CallejeroViaSearch;
...
{	
	...
	public static final String LIST = "/callejero/via";
	
	public static final String SEARCH_DISTINCT = LIST+"/distinct";
	
	public static final String RECORD = "/callejero/via/{id}";
	
	public static final String TRANSFORM = "/callejero/via/transform";
	
	public static final String ADD = "/callejero/via";
	public static final String UPDATE = "/callejero/via/{id}";
	public static final String DELETE = "/callejero/via/{id}";
	
	public static final String MODEL_VIEW_LIST = "callejero/viaList";
	public static final String MODEL_VIEW_ID = "callejero/viaId";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = CallejeroViaController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("VIA_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("VIA_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("VIA_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("VIA_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("VIA_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("VIA_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(CallejeroVia.class);

	private static final Logger log = LoggerFactory.getLogger(CallejeroViaController.class);
	

	@Autowired
	protected DatasetService<CallejeroVia> service;
	
	@Autowired
	private DatasetService<CallejeroTramoVia> callejeroTramoService;
	
	@Autowired
	private DatasetService<CallejeroPortal> callejeroPortalService;	
															...
															@RequestParam(value = Constants.PAGESIZE, defaultValue = Constants.defaultGroupByPageSize+"", required = false) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new CallejeroVia(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request, CallejeroViaSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<CallejeroVia>, EntityManager> visitor = new JpaCriteriaQueryVisitor<CallejeroVia>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new CallejeroVia(), new CallejeroViaResult(), 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=CallejeroViaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=CallejeroViaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=CallejeroViaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		try
		{			
			CallejeroVia calleToCheck=service.findById(CallejeroConstants.KEY, CallejeroVia.class, id);
			//si es nulo no hacemos ningún control adicional, porque ya se encarga el metodo generico
			if (calleToCheck!=null)
			{				
				errores = checkClavesExternas(calleToCheck,Constants.BASIC_OPERATION_DELETE);
			}
		} 
		...
		if (errores.size()>0)
		{
			...
			errorApi.setClassName(CallejeroViaController.class.getName());
		...
		{		
			return delete(id, new CallejeroVia(), nameController, DELETE, service,getKey());
		}
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_CALLEJEROVIA_TEXT) @RequestBody CallejeroVia obj) {
	...
	private List<String> checkClavesExternas(CallejeroVia via, String operation) throws DAOException
	{
		List<String> errores=new ArrayList<String>();		
		//Chequeamos los identificadores que apuntan a otras tablas
		CallejeroTramoViaSearch searchTramo=new CallejeroTramoViaSearch();
		searchTramo.setVia(via.getId());
		long rowcountTramos = callejeroTramoService.rowcount(CallejeroConstants.KEY, CallejeroTramoVia.class, searchTramo);								
		if (rowcountTramos>0)
		{
			errores.add("this via is used "+rowcountTramos+" times in tramosVia");
		}					
		
		CallejeroPortalSearch searchPortal=new CallejeroPortalSearch();
		searchPortal.setVia(via.getId());		
		long rowcountPortal = callejeroPortalService.rowcount(CallejeroConstants.KEY, CallejeroPortal.class, searchPortal);	
		if (rowcountPortal>0)
		{
			errores.add("this via is used "+rowcountPortal+" times in Portal");
		}
