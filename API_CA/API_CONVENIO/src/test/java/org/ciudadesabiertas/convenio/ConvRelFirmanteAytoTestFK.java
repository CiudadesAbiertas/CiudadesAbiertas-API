...
public class ConvRelFirmanteAytoTestFK {
    ...
    public void test_01_add() throws Exception {
    	...
    	{    	
    		String id ="TEST01_FIRMANTEAYTO01";
        	String item = "{"   			
        			+ "	  \"id\": \""+id+"\",\r\n"         			
	         ...
	         this.mockMvc.perform(MockMvcRequestBuilders.post(ConvRelFirmanteAytoController.ADD)
    	...
    	{    	
    		...
    		String item = "{"   			
	     	...
	     	item = new String (item.getBytes(),"UTF-8");
	     	 ...
	     	 this.mockMvc.perform(MockMvcRequestBuilders.put(ConvRelFirmanteAytoController.ADD+"/"+id)
	         		.contentType(MediaType.APPLICATION_JSON)
	     	        .content(item)).andExpect(MockMvcResultMatchers.status().isConflict());
    	}
	...
	public void test_99_Delete() throws Exception {
		if (activeFK == false) {
			...
			this.mockMvc
					.perform(MockMvcRequestBuilders.delete(ConvRelFirmanteAytoController.ADD + "/" + id)
							.contentType(MediaType.APPLICATION_JSON))
