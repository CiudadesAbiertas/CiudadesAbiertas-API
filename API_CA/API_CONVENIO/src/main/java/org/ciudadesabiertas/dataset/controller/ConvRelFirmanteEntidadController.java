...
import org.ciudadesabiertas.dataset.utils.ConvRelFirmanteEntidadResult;
...
@RestController
@Api(value="Convenio - Firmante Entidad",description = "Conjunto de operaciones relacionadas con el conjunto de datos Convenio - Firmante Entidad", tags= {"Convenio - Firmante Entidad"})
public class ConvRelFirmanteEntidadController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/convenio/firmante-entidad";
	...
	public static final String MODEL_VIEW_LIST = "convenio/firmanteEntidad/list";
	public static final String MODEL_VIEW_ID = "convenio/firmanteEntidad/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = ConvRelFirmanteEntidadController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("ConvRelFirmanteEntidad_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("ConvRelFirmanteEntidad_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("ConvRelFirmanteEntidad_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("ConvRelFirmanteEntidad_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("ConvRelFirmanteEntidad_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("ConvRelFirmanteEntidad_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(ConvRelFirmanteEntidad.class);

	private static final Logger log = LoggerFactory.getLogger(ConvRelFirmanteEntidadController.class);
		

	@Autowired
	protected DatasetService<ConvRelFirmanteEntidad> service;
	
	@Autowired
	protected DatasetService<ConvenioSuscEntidad> serviceEntidad;
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_DISTINCT,  response=ConvRelFirmanteEntidadResult.class),
	...
	public @ResponseBody ResponseEntity<?> distinctSearch(HttpServletRequest request, DistinctSearch search,															
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new ConvRelFirmanteEntidad(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,ConvRelFirmanteEntidadSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=ConvRelFirmanteEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, ConvRelFirmanteEntidadSearch search, 
		...
		RSQLVisitor<CriteriaQuery<ConvRelFirmanteEntidad>, EntityManager> visitor = new JpaCriteriaQueryVisitor<ConvRelFirmanteEntidad>();
		
		ResponseEntity<ConvRelFirmanteEntidad> list= list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new ConvRelFirmanteEntidad(), new ConvRelFirmanteEntidadResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, ConvRelFirmanteEntidadSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields,
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=ConvRelFirmanteEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_CONVENIOSRELFIRENTIDAD_TEXT) 			
			@RequestBody ConvRelFirmanteEntidad obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=ConvRelFirmanteEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=ConvRelFirmanteEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new ConvRelFirmanteEntidad(), nameController, DELETE, service,getKey());
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=ConvRelFirmanteEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, 
			@PathVariable 
			@ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_CONVENIOSRELFIRENTIDAD) String id)
	{
		...
		ResponseEntity<ConvRelFirmanteEntidad> record= record(request, id, new ConvRelFirmanteEntidad(),new ConvRelFirmanteEntidadResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_CONVENIOSRELFIRENTIDAD_TEXT) @RequestBody ConvRelFirmanteEntidad obj) {
...
//	@SuppressWarnings("unchecked")
//	private ResponseEntity<?> integraOrganigrama(ResponseEntity<ConvRelFirmanteEntidad> list,  HttpServletRequest request) {
//
...
//
//				Result<ConvRelFirmanteEntidad> result = ((Result<ConvRelFirmanteEntidad>) body);
//
//				List<ConvRelFirmanteEntidad> records = result.getRecords();
//
//				if (Util.isOrganigramaIntegration()) {
//					for (ConvRelFirmanteEntidad agenda : records) {
...
//	}
	...
	private List<String> checkClavesExternas(ConvRelFirmanteEntidad convFirmanteEntidad)
	{
		List<String> errores=new ArrayList<String>();
		//Chequeamos los identificadores que apuntan a otras tablas					
		if (Util.validValue(convFirmanteEntidad.getEntidadId()) && activeFK)
		{
		//1 CHECK
			ConvenioSuscEntidad findById = serviceEntidad.findById(getKey(),ConvenioSuscEntidad.class,convFirmanteEntidad.getEntidadId());
			if (findById==null)
			{
				errores.add("Entidad ID not exists in entity: "+convFirmanteEntidad.getEntidadId());
			}
			
		//2 CHECK
			ConvenioOrganization findOrganizationById = serviceOrg.findById(getKey(),ConvenioOrganization.class,convFirmanteEntidad.getOrganizationId());
			if (findOrganizationById==null)
			{
				errores.add("Organization ID not exists in entity: "+convFirmanteEntidad.getOrganizationId());
			}
		}
