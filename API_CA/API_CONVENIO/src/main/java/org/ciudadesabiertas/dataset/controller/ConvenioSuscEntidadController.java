...
import org.ciudadesabiertas.dataset.utils.ConvenioSuscEntidadResult;
...
@RestController
@Api(value="Convenio - Suscripción por Entidad",description = "Conjunto de operaciones relacionadas con el conjunto de datos Convenio - Suscripción por Entidad", tags= {"Convenio - Suscripción por Entidad"})
public class ConvenioSuscEntidadController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/convenio/suscripcion-por-entidad";
	...
	public static final String MODEL_VIEW_LIST = "convenio/entidad/list";
	public static final String MODEL_VIEW_ID = "convenio/entidad/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = ConvenioSuscEntidadController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("ConvenioSuscEntidad_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("ConvenioSuscEntidad_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("ConvenioSuscEntidad_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("ConvenioSuscEntidad_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("ConvenioSuscEntidad_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("ConvenioSuscEntidad_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(ConvenioSuscEntidad.class);

	private static final Logger log = LoggerFactory.getLogger(ConvenioSuscEntidadController.class);
		

	@Autowired
	protected DatasetService<ConvenioSuscEntidad> service;
	
	@Autowired
	protected DatasetService<Convenio> serviceEvento;
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_DISTINCT,  response=ConvenioSuscEntidadResult.class),
	...
	public @ResponseBody ResponseEntity<?> distinctSearch(HttpServletRequest request, DistinctSearch search,															
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new ConvenioSuscEntidad(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,ConvenioSuscEntidadSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=ConvenioSuscEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, ConvenioSuscEntidadSearch search, 
		...
		RSQLVisitor<CriteriaQuery<ConvenioSuscEntidad>, EntityManager> visitor = new JpaCriteriaQueryVisitor<ConvenioSuscEntidad>();
		
		ResponseEntity<ConvenioSuscEntidad> list= list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new ConvenioSuscEntidad(), new ConvenioSuscEntidadResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, ConvenioSuscEntidadSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields,
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=ConvenioSuscEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_CONVENIOSUSCENTIDAD_TEXT) 			
			@RequestBody ConvenioSuscEntidad obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=ConvenioSuscEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=ConvenioSuscEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new ConvenioSuscEntidad(), nameController, DELETE, service,getKey());
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=ConvenioSuscEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request,
			@PathVariable 
			@ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_CONVENIOSUSENTIDAD) String id)
	{
		...
		ResponseEntity<ConvenioSuscEntidad> record= record(request, id, new ConvenioSuscEntidad(),new ConvenioSuscEntidadResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_CONVENIOSUSCENTIDAD_TEXT) @RequestBody ConvenioSuscEntidad obj) {
...
//	@SuppressWarnings("unchecked")
//	private ResponseEntity<?> integraOrganigrama(ResponseEntity<ConvenioSuscEntidad> list,  HttpServletRequest request) {
//
...
//
//				Result<ConvenioSuscEntidad> result = ((Result<ConvenioSuscEntidad>) body);
//
//				List<ConvenioSuscEntidad> records = result.getRecords();
//
//				if (Util.isOrganigramaIntegration()) {
//					for (ConvenioSuscEntidad agenda : records) {
...
//	}
	...
	private List<String> checkClavesExternas(ConvenioSuscEntidad conSusEnt)
	{
		List<String> errores=new ArrayList<String>();
		//Chequeamos los identificadores que apuntan a otras tablas					
		if (Util.validValue(conSusEnt.getConvenioId()) && activeFK)
		{
		//1FK
			Convenio findById = serviceEvento.findById(getKey(),Convenio.class,conSusEnt.getConvenioId());
			if (findById==null)
			{
				errores.add("Convenio ID not exists in entity: "+conSusEnt.getConvenioId());
			}
		//2FK
			ConvenioOrganization findByOrgId = serviceOrg.findById(getKey(),ConvenioOrganization.class,conSusEnt.getOrganizationId());
			if (findByOrgId==null)
			{
				errores.add("Organization ID not exists in entity: "+conSusEnt.getOrganizationId());
			}
		}
