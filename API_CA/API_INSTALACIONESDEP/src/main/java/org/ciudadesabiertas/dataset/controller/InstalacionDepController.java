...
import org.ciudadesabiertas.dataset.utils.InstalacionDepGeoSearch;
import org.ciudadesabiertas.dataset.utils.InstalacionDepResult;
import org.ciudadesabiertas.dataset.utils.InstalacionDepSearch;
...
@RestController
@Api(value="InstalacionDeportiva",description = "Conjunto de operaciones relacionadas con el conjunto de datos de Instalacion Deportiva. Utiliza el mismo modelo que Equipamientos. "+SwaggerConstants.VOCABULARIO_A_HREF+InstalacionDepConstants.equipamientoVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Instalación Deportiva"})
public class InstalacionDepController extends GenericController implements CiudadesAbiertasController
{
	public static final String LIST = "/instalacion-deportiva/instalacion-deportiva";
	...
	public static final String MODEL_VIEW_LIST = "instalacionDep/list";
	public static final String MODEL_VIEW_ID = "instalacionDep/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = InstalacionDepController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("EQID_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("EQID_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("EQID_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("EQID_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("EQID_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("EQID_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(Equipamiento.class);

	private static final Logger log = LoggerFactory.getLogger(InstalacionDepController.class);
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		search.setCondicionAdicional(" "+InstalacionDepConstants.TIPO_EQUIPAMIENTO_FIELD+ " like '"+InstalacionDepConstants.TIPO_EQUIPAMIENTO+"' ");
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,InstalacionDepSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=InstalacionDepResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {GEO_LIST,  VERSION_1+GEO_LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> geoList(HttpServletRequest request,	InstalacionDepGeoSearch search, 
	...
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, InstalacionDepSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		if (Util.validValue(rsqlQ)) {
			if (!rsqlQ.contains(InstalacionDepConstants.TIPO_EQUIPAMIENTO_FIELD)) {
				rsqlQ += " and " + InstalacionDepConstants.TIPO_EQUIPAMIENTO_FIELD + "=='"
						+ InstalacionDepConstants.TIPO_EQUIPAMIENTO + "'";
			}
		}

		if (search != null) {
			search.setTipoEquipamiento(InstalacionDepConstants.TIPO_EQUIPAMIENTO);
		} else {
			search = new InstalacionDepSearch();
			search.setTipoEquipamiento(InstalacionDepConstants.TIPO_EQUIPAMIENTO);
		}
		...
		ResponseEntity list= list(request, search, fields, rsqlQ, page, pageSize, sort, srId, LIST, new Equipamiento(),
				new InstalacionDepResult(), availableFields, getKey(), visitor, service);
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, InstalacionDepSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=InstalacionDepResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=InstalacionDepResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_TEXT+SwaggerConstants.PARAM_ID_INSTALACION_DEPORTIVA) 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=InstalacionDepResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=InstalacionDepResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_INSTALACION_DEPORTIVA) String id, @RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value=SwaggerConstants.PARAM_SRID, allowableValues=Constants.SUPPORTED_SRIDS)  String srId)
	{
		...
		ResponseEntity record = record(request, id, new Equipamiento(),new InstalacionDepResult(), srId, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_INSTALACION_DEPORTIVA) String id, @RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value=SwaggerConstants.PARAM_SRID, allowableValues=Constants.SUPPORTED_SRIDS)  String srId	)
	{
	...
	{
		return InstalacionDepConstants.KEY;
	}
	...
	private boolean checkTipoEquipamineto(Equipamiento obj) {
		...
		if (obj != null) {
			if (Util.validValue(obj.getTipoEquipamiento())) {
				if (InstalacionDepConstants.TIPO_EQUIPAMIENTO.equals(obj.getTipoEquipamiento())) {
					resultado = true;
				}
			}
		}
	...
	private ResponseEntity<?> generaErrorTipoEquipamiento(String operacion, String tipoEquipamiento) {
		...
		String error = "[ERROR Validator: Error en el Tipo Equipamiento]" + "[tipoEquipamiento(Esperado):"
				+ InstalacionDepConstants.TIPO_EQUIPAMIENTO + "]" + "[tipoEquipamiento(Recibido):" + tipoEquipamiento
				+ "]";
