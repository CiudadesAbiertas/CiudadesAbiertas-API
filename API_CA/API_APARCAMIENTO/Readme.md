...
# API CIUDADES ABIERTAS - MÓDULO APARCAMIENTOS

Esta es la documentación asociada al módulo de Punto Wifi.
...
Para realizar esto lo único que se necesita es crear un fichero **aparcamiento.properties** e incluirlo en la carpeta de recursos del proyecto WEB (API_WEB/src/main/resources/) donde se distribuyen el resto de ficheros de configuración.

Esto permite que el módulo de **aparcamientos** utilice la información que contenga este fichero cuando se ejecutan peticiones sobre dicho módulo.

A continuación se detallan los parámetros que se pueden configurar en este fichero **aparcamiento.properties**.
...
- **EQAP_LIST**: Listado de aparcamientos
- **EQAP_RECORD**: Ficha de aparcamiento
- **EQAP_ADD**:  Alta de aparcamiento
- **EQAP_UPDATE**: Modificación de un aparcamiento
- **EQAP_DELETE**: Baja de aparcamiento
- **EQAP_TRANSFORM**: Transformador de un aparcamiento externo (a través de JSON)

Y los parámetros configurados quedarían de la siguiente manera en el fichero (**aparcamiento.properties**) Como Ejemplo:
- **peticiones.identificadas.public_auth=EQAP_ADD,EQAP_RECORD,EQAP_LIST**
- **peticiones.identificadas.basic_auth= EQAP_UPDATE,EQAP_TRANSFORM**
