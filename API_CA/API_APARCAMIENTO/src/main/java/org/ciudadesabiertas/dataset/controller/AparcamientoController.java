...
import org.ciudadesabiertas.dataset.utils.AparcamientoGeoSearch;
import org.ciudadesabiertas.dataset.utils.AparcamientoResult;
import org.ciudadesabiertas.dataset.utils.AparcamientoSearch;
...
@RestController
@Api(value="Aparcamiento",description = "Conjunto de operaciones relacionadas con el conjunto de datos Aparcamiento. Utiliza el mismo modelo que Equipamientos."+SwaggerConstants.VOCABULARIO_A_HREF+AparcamientoConstants.equipamientoVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Aparcamiento"})
public class AparcamientoController extends GenericController implements CiudadesAbiertasController
{
	public static final String LIST = "/aparcamiento/aparcamiento";
	
	public static final String SEARCH_DISTINCT = LIST+"/distinct";
	
	public static final String GEO_LIST = LIST+ "/geo";
	
	public static final String RECORD = "/aparcamiento/aparcamiento/{id}";
	
	public static final String TRANSFORM = "/aparcamiento/aparcamiento/transform";
	
		
	public static final String ADD = "/aparcamiento/aparcamiento";
	public static final String UPDATE = "/aparcamiento/aparcamiento/{id}";
	public static final String DELETE = "/aparcamiento/aparcamiento/{id}";
	
	public static final String MODEL_VIEW_LIST = "aparcamiento/list";
	public static final String MODEL_VIEW_ID = "aparcamiento/id";
		
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(Equipamiento.class);

	private static final Logger log = LoggerFactory.getLogger(AparcamientoController.class);
	...
	private static String nameController = AparcamientoController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("EQAP_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("EQAP_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("EQAP_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("EQAP_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("EQAP_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("EQAP_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
				...
				@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		search.setCondicionAdicional(" "+AparcamientoConstants.TIPO_EQUIPAMIENTO_FIELD+ " like '"+AparcamientoConstants.TIPO_EQUIPAMIENTO+"' ");
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,AparcamientoSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=AparcamientoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {GEO_LIST,  VERSION_1+GEO_LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> geoList(HttpServletRequest request,	AparcamientoGeoSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		ResponseEntity list= geoList(request, search, fields, meters, page, pageSize, sort, srId, LIST, new Equipamiento(), new AparcamientoResult(), availableFields, getKey(),service);
	...
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, AparcamientoSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		if (Util.validValue(rsqlQ)) {			
			if (!rsqlQ.contains(AparcamientoConstants.TIPO_EQUIPAMIENTO_FIELD)) {
				rsqlQ += " and " + AparcamientoConstants.TIPO_EQUIPAMIENTO_FIELD + "=='"+AparcamientoConstants.TIPO_EQUIPAMIENTO+"'";
			}
		}
		if (search!=null){
			search.setTipoEquipamiento(AparcamientoConstants.TIPO_EQUIPAMIENTO);				
		}else {
			search = new AparcamientoSearch();
			search.setTipoEquipamiento(AparcamientoConstants.TIPO_EQUIPAMIENTO);
		}
		//FIN CODIGO PERSONALIZADO
		
		ResponseEntity list= list(request, search, fields, rsqlQ, page, pageSize, sort, srId, LIST,new Equipamiento(), new AparcamientoResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, AparcamientoSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=AparcamientoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=AparcamientoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_TEXT+SwaggerConstants.PARAM_ID_APARCAMIENTO) 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=AparcamientoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=AparcamientoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_APARCAMIENTO) String id, @RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value=SwaggerConstants.PARAM_SRID, allowableValues=Constants.SUPPORTED_SRIDS)  String srId)
	{
		...
		ResponseEntity record = record(request, id, new Equipamiento(),new AparcamientoResult(), srId, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_APARCAMIENTO) String id, @RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value=SwaggerConstants.PARAM_SRID, allowableValues=Constants.SUPPORTED_SRIDS)  String srId	)
	{
	...
	{
		return AparcamientoConstants.KEY;
	}
	...
	private boolean checkTipoEquipamineto (Equipamiento obj) {
		...
		if (obj!=null) {
			if (Util.validValue(obj.getTipoEquipamiento())) {
				if (AparcamientoConstants.TIPO_EQUIPAMIENTO.equals(obj.getTipoEquipamiento())){
					resultado=true;
				}
			}
		}
	...
	private ResponseEntity<?> generaErrorTipoEquipamiento(String operacion, String tipoEquipamiento) {
		...
		String error = "[ERROR Validator: Error en el Tipo Equipamiento]" + "[tipoEquipamiento(Esperado):"
				+ AparcamientoConstants.TIPO_EQUIPAMIENTO + "]" + "[tipoEquipamiento(Recibido):" + tipoEquipamiento
				+ "]";
