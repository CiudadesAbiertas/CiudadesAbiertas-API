...
import org.ciudadesabiertas.dataset.utils.TraficoPropiedadMedicionResult;
import org.ciudadesabiertas.dataset.utils.TraficoPropiedadMedicionSearch;
...
@RestController
@Api(value="TraficoPropiedadMedicion",description = "Conjunto de operaciones relacionadas con el conjunto de datos Trafico"+SwaggerConstants.VOCABULARIO_A_HREF+TraficoConstants.traficoVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Tráfico - Propiedad Medición"})
public class TraficoPropiedadMedicionController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/trafico/propiedad-medicion";
	...
	public static final String MODEL_VIEW_LIST = "trafico/propiedadMedicion/list";
	public static final String MODEL_VIEW_ID = "trafico/propiedadMedicion/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = TraficoPropiedadMedicionController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("TraficoPropiedadMedicion_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("TraficoPropiedadMedicion_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("TraficoPropiedadMedicion_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("TraficoPropiedadMedicion_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("TraficoPropiedadMedicion_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("TraficoPropiedadMedicion_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(TraficoPropiedadMedicion.class);
	{
		availableFields.add(Constants.XETRS89Fin);
		availableFields.add(Constants.YETRS89Fin);
	}

	private static final Logger log = LoggerFactory.getLogger(TraficoPropiedadMedicionController.class);
		

	@Autowired
	protected DatasetService<TraficoPropiedadMedicion> service;
	@Autowired
				...
				@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new TraficoPropiedadMedicion(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,TraficoPropiedadMedicionSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=TraficoPropiedadMedicionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, TraficoPropiedadMedicionSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<TraficoPropiedadMedicion>, EntityManager> visitor = new JpaCriteriaQueryVisitor<TraficoPropiedadMedicion>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new TraficoPropiedadMedicion(), new TraficoPropiedadMedicionResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, TraficoPropiedadMedicionSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false)
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false)
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
...
//	@ApiResponses({
//	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=TraficoPropiedadMedicionResult.class),
//	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
//	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
//	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
//	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
...
//			@RequestBody TraficoPropiedadMedicion obj 
//			)
//	{
//
//		log.info("[add][" + ADD + "]");
...
//	@ApiResponses({
//	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=TraficoPropiedadMedicionResult.class),
//	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
//	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
//	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
//	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
...
//	@ApiResponses({
//	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=TraficoPropiedadMedicionResult.class),
//	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
//	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
//	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
//	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
...
//
//		
//		return delete(id, new TraficoPropiedadMedicion(), nameController, DELETE, service,getKey());
//	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=TraficoPropiedadMedicionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_TRAFICO_PROP_MEDI) String id)
	{
		...
		return record(request, id, new TraficoPropiedadMedicion(),new TraficoPropiedadMedicionResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_TRAFICO_PROP_MEDI) String id)
	{
...
//			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_PLANTILLA_TEXT) @RequestBody TraficoPropiedadMedicion obj) {
//
//		log.info("[transform]");
//
