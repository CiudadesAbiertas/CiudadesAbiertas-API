...
import org.ciudadesabiertas.dataset.utils.TraficoTramoGeoSearch;
import org.ciudadesabiertas.dataset.utils.TraficoTramoResult;
...
@RestController
@Api(value="TraficoTramo",description = "Conjunto de operaciones relacionadas con el conjunto de datos Trafico"+SwaggerConstants.VOCABULARIO_A_HREF+TraficoConstants.traficoVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Tráfico - Tramo"})
public class TraficoTramoController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/trafico/tramo";
	...
	public static final String MODEL_VIEW_LIST = "trafico/tramo/list";
	public static final String MODEL_VIEW_ID = "trafico/tramo/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = TraficoTramoController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("TraficoTramo_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("TraficoTramo_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("TraficoTramo_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("TraficoTramo_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("TraficoTramo_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("TraficoTramo_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
	...
	@Autowired
	protected DatasetService<TraficoTramo> service;
	@Autowired
	protected DatasetService<TraficoDispositivoMedicion> dispositivoService;
	@Autowired
	protected DatasetService<TraficoIncidencia> incidenciaService;
	@Autowired
	protected DatasetService<TraficoObservacion> observacionService;
	@Autowired
	protected DatasetService<TraficoTramoVia> tramoViaService;
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=TraficoTramoResult.class),
				...
				@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new TraficoTramo(), new ObjectResult(),  service);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=TraficoTramoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=TraficoTramoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=TraficoTramoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new TraficoTramo(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=TraficoTramoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	private List<String> checkClavesExternas(String tramoId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				if (tramoId!=null)
				{
					TraficoDispositivoMedicionSearch search=new TraficoDispositivoMedicionSearch();
					search.setMonitorea(tramoId);
					long rowcountDisp = dispositivoService.rowcount(getKey(), TraficoDispositivoMedicion.class, search);
					
					TraficoIncidenciaSearch search2 = new TraficoIncidenciaSearch();
					search2.setIncidenciaEnTramo(tramoId);
					...
					search4.setIdTramo(tramoId);
					long rowcountTramV = tramoViaService.rowcount(getKey(), TraficoTramoVia.class, search4);
					
					if (rowcountDisp>0 || rowcountInc>0 || rowcountObs>0 || rowcountTramV>0)			
					{
						errors.add("El tramo '"+tramoId+"' está siendo usado");
					}
