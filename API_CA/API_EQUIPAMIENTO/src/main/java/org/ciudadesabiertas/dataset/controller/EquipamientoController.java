...
import org.ciudadesabiertas.dataset.utils.EquipamientoGeoSearch;
import org.ciudadesabiertas.dataset.utils.EquipamientoResult;
...
public class EquipamientoController extends GenericController implements CiudadesAbiertasController {
	...
	public static final String LIST = "/equipamiento/equipamiento";
	
	public static final String SEARCH_DISTINCT = LIST+"/distinct";
	
	public static final String GEO_LIST = LIST+ "/geo";

	public static final String RECORD = "/equipamiento/equipamiento/{id}";

	public static final String TRANSFORM = "/equipamiento/equipamiento/transform";

	public static final String ADD = "/equipamiento/equipamiento";
	public static final String UPDATE = "/equipamiento/equipamiento/{id}";
	public static final String DELETE = "/equipamiento/equipamiento/{id}";

	public static final String MODEL_VIEW_LIST = "equipamiento/list";
	public static final String MODEL_VIEW_ID = "equipamiento/id";

	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = EquipamientoController.class.getName();

	// Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("EQ_LIST", LIST, HttpMethod.GET, Constants.NO_AUTH));
		listRequestType.add(new RequestType("EQ_RECORD", RECORD, HttpMethod.GET, Constants.NO_AUTH));
		listRequestType.add(new RequestType("EQ_TRANSFORM", TRANSFORM, HttpMethod.POST, Constants.NO_AUTH));

		listRequestType.add(new RequestType("EQ_ADD", ADD, HttpMethod.POST, Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("EQ_UPDATE", UPDATE, HttpMethod.PUT, Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("EQ_DELETE", DELETE, HttpMethod.DELETE, Constants.BASIC_AUTH));

	}

	public static List<String> availableFields = Util.extractPropertiesFromBean(Equipamiento.class);

	private static final Logger log = LoggerFactory.getLogger(EquipamientoController.class);
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		search.setCondicionAdicional(" "+EquipamientoConstants.TIPO_EQUIPAMIENTO_FIELD+ " like '"+EquipamientoConstants.TIPO_EQUIPAMIENTO+"' ");
	...
	public ModelAndView listHTML(ModelAndView mv, HttpServletRequest request, EquipamientoSearch search,
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=EquipamientoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		log.debug("[parmam] [page:" + page + "] "
						+ "[pageSize:" + pageSize + "] "
						+ "[fields:" + fields + "] "
						+ "[meters:" + meters + "] "
						+ "[xETRS89:" + search.getxETRS89() + "] "
						+ "[yETRS89:" + search.getyETRS89() + "] "
						+ "[sort:" + sort + "]");
	...
	@SuppressWarnings("unchecked")
	@ApiOperation(value = SwaggerConstants.LISTADO_Y_BUSQUEDA, notes = SwaggerConstants.DESCRIPCION_BUSQUEDA, produces = SwaggerConstants.FORMATOS_CONSULTA_RESPONSE_FULL, authorizations = {
			@Authorization(value = Constants.APIKEY) })
	@ApiResponses({
			@ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO, response = EquipamientoResult.class),
			@ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA, response = ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, EquipamientoSearch search,
		...
		if (Util.validValue(rsqlQ)) {
			if (!rsqlQ.contains(EquipamientoConstants.TIPO_EQUIPAMIENTO_FIELD)) {
				rsqlQ += " and " + EquipamientoConstants.TIPO_EQUIPAMIENTO_FIELD + "=='"
						+ EquipamientoConstants.TIPO_EQUIPAMIENTO + "'";
			}
		}

		if (search != null) {
			search.setTipoEquipamiento(EquipamientoConstants.TIPO_EQUIPAMIENTO);
		} else {
			search = new EquipamientoSearch();
			search.setTipoEquipamiento(EquipamientoConstants.TIPO_EQUIPAMIENTO);
		}
	...
	@ApiOperation(value = SwaggerConstants.INSERCION, notes = SwaggerConstants.DESCRIPCION_INSERCION, produces = SwaggerConstants.FORMATOS_ADD_RESPONSE, consumes = SwaggerConstants.FORMATOS_ADD_REQUEST, authorizations = {
			@Authorization(value = Constants.APIKEY) })
	@ApiResponses({
			@ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION, response = EquipamientoResult.class),
			@ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA, response = ResultError.class),
			@ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO, response = ResultError.class),
			@ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE, response = ResultError.class),
			@ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO, response = ResultError.class) })
	...
	@ApiOperation(value = SwaggerConstants.MODIFICACION, notes = SwaggerConstants.DESCRIPCION_MODIFICACION, produces = SwaggerConstants.FORMATOS_ADD_RESPONSE, consumes = SwaggerConstants.FORMATOS_ADD_REQUEST, authorizations = {
			@Authorization(value = Constants.APIKEY) })
	@ApiResponses({
			@ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION, response = EquipamientoResult.class),
			@ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA, response = ResultError.class),
			@ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO, response = ResultError.class),
			@ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE, response = ResultError.class),
			@ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO, response = ResultError.class) })
	...
	@ApiOperation(value = SwaggerConstants.DELETE, notes = SwaggerConstants.DESCRIPCION_DELETE, produces = SwaggerConstants.FORMATOS_ADD_RESPONSE, consumes = SwaggerConstants.FORMATOS_ADD_REQUEST, authorizations = {
			@Authorization(value = Constants.APIKEY) })
	@ApiResponses({
			@ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE, response = EquipamientoResult.class),
			@ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA, response = ResultError.class),
			@ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO, response = ResultError.class),
			@ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE, response = ResultError.class),
			@ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO, response = ResultError.class) })
	...
	@SuppressWarnings("unchecked")
	@ApiOperation(value = SwaggerConstants.FICHA, notes = SwaggerConstants.DESCRIPCION_FICHA, produces = SwaggerConstants.FORMATOS_CONSULTA_RESPONSE_FULL, authorizations = {
			@Authorization(value = Constants.APIKEY) })
	@ApiResponses({
			@ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA, response = EquipamientoResult.class),
			@ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA, response = ResultError.class),
			@ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO, response = ResultError.class),
			@ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE, response = ResultError.class),
			@ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO, response = ResultError.class) })
	...
	@ApiOperation(value = SwaggerConstants.FICHA, notes = SwaggerConstants.DESCRIPCION_FICHA_HEAD, produces = SwaggerConstants.FORMATOS_CONSULTA_RESPONSE_NO_HTML, authorizations = {
			@Authorization(value = Constants.APIKEY) })
	@ApiResponses({
			@ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA, response = EquipamientoResult.class),
			@ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA, response = ResultError.class),
	...
	public String getKey() {
		return EquipamientoConstants.KEY;
	}
	...
	private boolean checkTipoEquipamineto(Equipamiento obj) {
		...
		if (obj != null) {
			if (Util.validValue(obj.getTipoEquipamiento())) {
				if (EquipamientoConstants.TIPO_EQUIPAMIENTO.equals(obj.getTipoEquipamiento())) {
					resultado = true;
				}
			}
		}
	...
	private ResponseEntity<?> generaErrorTipoEquipamiento(String operacion, String tipoEquipamiento) {
		...
		String error = "[ERROR Validator: Error en el Tipo Equipamiento]" + "[tipoEquipamiento(Esperado):"
				+ EquipamientoConstants.TIPO_EQUIPAMIENTO + "]" + "[tipoEquipamiento(Recibido):" + tipoEquipamiento
				+ "]";
