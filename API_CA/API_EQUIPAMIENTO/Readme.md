...
# API CIUDADES ABIERTAS - MÓDULO EQUIPAMIENTO

Esta es la documentación asociada al módulo de Equipamiento.
...
Para realizar esto lo único que se necesitam es crear un fichero **equipamiento.properties** e incluirlo en la carpeta de recursos del proyecto WEB (API_WEB/src/main/resources/) donde se distribuyen el resto de ficheros de configuración.

Esto permite que el módulo de *equipamiento* utilice la información que contenga este fichero cuando se ejecutan peticiones sobre dicho módulo.

A continuación se detallan los parámetros que se pueden configurar en este fichero **equipamiento.properties**.
...
- **EQ_LIST**: Listado de equipamientos
- **EQ_RECORD**: Ficha de equipamiento
- **EQ_ADD**:  Alta de equipamiento
- **EQ_UPDATE**: Modificación de equipamiento
- **EQ_DELETE**: Baja de equipamiento
- **EQ_TRANSFORM**: Transformador de un equipamiento externo (a través de JSON)

Y los parámetros configurados quedarían de la siguiente manera en el fichero (**equipamiento.properties**) Como Ejemplo:
- **peticiones.identificadas.public_auth=EQ_ADD,EQ_RECORD,EQ_LIST**
- **peticiones.identificadas.basic_auth= EQ_UPDATE,EQ_TRANSFORM**
- **peticiones.identificadas.admin_auth= EQ_DELETE**
