...
# API CIUDADES ABIERTAS - MÓDULO DE MONUMENTOS

Esta es la documentación asociada al módulo de Monumentos.
...
Para realizar esto lo único que se necesita es crear un fichero **monumento.properties** e incluirlo en la carpeta de recursos del proyecto WEB (API_WEB/src/main/resources/) donde se distribuyen el resto de ficheros de configuración.

Esto permite que el módulo de **monumento** utilice la información que contenga este fichero cuando se ejecutan peticiones sobre dicho módulo.

A continuación se detallan los parámetros que se pueden configurar en este fichero **monumento.properties**.
...
Cuando se implementa esta funcionalidad se configura a nivel de etiquetas de los servicios de cada uno de los controladores asociados al módulo de monumento:
- **PITM_LIST**: Listado de monumentos
- **PITM_RECORD**: Ficha de monumento
- **PITM_ADD**:  Alta de monumento
- **PITM_UPDATE**: Modificación de monumento
- **PITM_DELETE**: Baja de monumento
- **PITM_TRANSMORM**: Transformador de una monumento externo (a través de JSON)

Y los parámetros configurados quedarían de la siguiente manera en el fichero (**interesTuristicos.properties**) Como Ejemplo:
- **peticiones.identificadas.public_auth=PITM_ADD,PITM_RECORD,PITM_LIST**
- **peticiones.identificadas.basic_auth= PITM_UPDATE,PITM_TRANSFORM**
- **peticiones.identificadas.admin_auth= PITM_DELETE**
