...
import org.ciudadesabiertas.dataset.utils.PuntoInteresTuristicoGeoSearch;
import org.ciudadesabiertas.dataset.utils.PuntoInteresTuristicoResult;
...
@RestController
@Api(value="PuntoInteresTuristico",description = "Conjunto de operaciones relacionadas con el conjunto de datos PuntoInteresTuristico"+SwaggerConstants.VOCABULARIO_A_HREF+PuntoInteresTuristicoConstants.lugarInteresTuristicoVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Punto de Interés Turístico"})
public class PuntoInteresTuristicoController extends GenericController implements CiudadesAbiertasController
{
	public static final String LIST = "/punto-interes-turistico/punto-interes-turistico";
	...
	public static final String MODEL_VIEW_LIST = "puntoInteresTuristico/list";
	public static final String MODEL_VIEW_ID = "puntoInteresTuristico/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = PuntoInteresTuristicoController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("PIT_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("PIT_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("PIT_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("PIT_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("PIT_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("PIT_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(PuntoInteresTuristico.class);

	private static final Logger log = LoggerFactory.getLogger(PuntoInteresTuristicoController.class);
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		search.setCondicionAdicional(" "+PuntoInteresTuristicoConstants.CATEGORY_FIELD+ " like '"+PuntoInteresTuristicoConstants.CATEGORY+"' ");
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,PuntoInteresTuristicoSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=PuntoInteresTuristicoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {GEO_LIST,  VERSION_1+GEO_LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> geoList(HttpServletRequest request,	PuntoInteresTuristicoGeoSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		ResponseEntity list= geoList(request, search, fields, meters, page, pageSize, sort, srId, LIST, new PuntoInteresTuristico(), new PuntoInteresTuristicoResult(), availableFields, getKey(),dsService);
	...
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, PuntoInteresTuristicoSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		if (Util.validValue(rsqlQ)) {
			if (!rsqlQ.contains(PuntoInteresTuristicoConstants.CATEGORY_FIELD)) {
				rsqlQ += " and " + PuntoInteresTuristicoConstants.CATEGORY_FIELD + "=='"
						+ PuntoInteresTuristicoConstants.CATEGORY + "'";
			}
		}

		if (search == null) {
			search = new PuntoInteresTuristicoSearch();
		}			
		search.setCategory(PuntoInteresTuristicoConstants.CATEGORY);
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, PuntoInteresTuristicoSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=PuntoInteresTuristicoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=PuntoInteresTuristicoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_TEXT+SwaggerConstants.PARAM_ID_PUNTO_INTERES_TURISTICO) 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=PuntoInteresTuristicoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new PuntoInteresTuristico(), PuntoInteresTuristicoController.class.getName(), DELETE, dsService,getKey());
	}	
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=PuntoInteresTuristicoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PUNTO_INTERES_TURISTICO) String id, @RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value=SwaggerConstants.PARAM_SRID, allowableValues=Constants.SUPPORTED_SRIDS)  String srId	)
	{
	...
	@ApiResponses({
		...
		return transform(obj,  PuntoInteresTuristicoController.class.getName(), TRANSFORM);
	...
	{
		return PuntoInteresTuristicoConstants.KEY;
	}
	...
	// METODOS PROPIOS PARA LOS MODULOS DE EQUIPAMIENTOS
		...
		private boolean checkCategory(PuntoInteresTuristico obj) {
			...
			if (obj != null) {
				if (Util.validValue(obj.getCategory())) {
					if (PuntoInteresTuristicoConstants.CATEGORY.equals(obj.getCategory())) {
						resultado = true;
					}
				}
			}
		...
		private ResponseEntity<?> generaErrorCategory(String operacion, String category) {
			...
			String error = "[ERROR Validator: Error field category]" + "[category(requiered):"
					+ PuntoInteresTuristicoConstants.CATEGORY + "]" + "[category(recive):" + category
					+ "]";
