...
# API CIUDADES ABIERTAS - MÓDULO INTERES TURISTICOS

Esta es la documentación asociada al módulo de Punto de Interes Turistico.
...
Para realizar esto lo único que se necesita es crear un fichero **interesTuristico.properties** e incluirlo en la carpeta de recursos del proyecto WEB (API_WEB/src/main/resources/) donde se distribuyen el resto de ficheros de configuración.

Esto permite que el módulo de **punto de interés turístico** utilice la información que contenga este fichero cuando se ejecutan peticiones sobre dicho módulo.

A continuación se detallan los parámetros que se pueden configurar en este fichero **interesTuristico.properties**.
...
Cuando se implementa esta funcionalidad se configura a nivel de etiquetas de los servicios de cada uno de los controladores asociados al módulo de Interes Turístico:
- **PIT_LIST**: Listado de puntos de interes turístico
- **PIT_RECORD**: Ficha de un punto de interes turístico
- **PIT_ADD**:  Alta de un punto de interes turístico
- **PIT_UPDATE**: Modificación de un punto de interes turístico
- **PIT_DELETE**: Baja de un punto de interes turístico
- **PIT_TRANSFORM**: Transformador de un punto de interes turístico externo (a través de JSON)

Y los parámetros configurados quedarían de la siguiente manera en el fichero (**interesTuristicos.properties**) Como Ejemplo:
- **peticiones.identificadas.public_auth=PIT_ADD,PIT_RECORD,PIT_LIST**
- **peticiones.identificadas.basic_auth= PIT_UPDATE,PIT_TRANSFORM**
- **peticiones.identificadas.admin_auth= PIT_DELETE**
