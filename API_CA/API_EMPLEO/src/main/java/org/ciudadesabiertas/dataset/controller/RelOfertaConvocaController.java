...
import org.ciudadesabiertas.dataset.model.BoletinOficial;
...
import org.ciudadesabiertas.dataset.model.RelBoletinConvoca;
import org.ciudadesabiertas.dataset.model.RelOfertaConvoca;
import org.ciudadesabiertas.dataset.utils.RelOfertaConvocaResult;
import org.ciudadesabiertas.dataset.utils.RelOfertaConvocaSearch;
...
@RestController
@Api(value="Relación Oferta y Convocatoria",description = "Conjunto de operaciones relacionadas con el conjunto de datos Relación entre Oferta y Convocatoria en Empleo"+SwaggerConstants.VOCABULARIO_A_HREF+EmpleoConstants.vocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Empleo - Relación Oferta y Convocatoria"})
public class RelOfertaConvocaController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/empleo/relacion-oferta-convocatoria";
	...
	public static final String MODEL_VIEW_LIST = "empleo/relOfertaConvocatoria/list";
	public static final String MODEL_VIEW_ID = "empleo/relOfertaConvocatoria/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = RelOfertaConvocaController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("RelOfertaConvoca_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("RelOfertaConvoca_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("RelOfertaConvoca_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("RelOfertaConvoca_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("RelOfertaConvoca_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("RelOfertaConvoca_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(RelOfertaConvoca.class);

	private static final Logger log = LoggerFactory.getLogger(RelOfertaConvocaController.class);
		

	@Autowired
	protected DatasetService<RelOfertaConvoca> service;
	
	@Autowired
	protected DatasetService<ConvocatoriaEmpleoPublico> convocatoriaService;
	
	@Autowired
	protected DatasetService<OfertaEmpleoPublico> ofertaService;

	
	@SuppressWarnings("unchecked")
	@ApiOperation(value = SwaggerConstants.BUSQUEDA_DISTINCT, notes = SwaggerConstants.FORMATOS_CONSULTA_RESPONSE_GROUPBY, produces = SwaggerConstants.FORMATOS_CONSULTA_RESPONSE_NO_HTML, authorizations = { @Authorization(value=Constants.APIKEY) })
	@ApiResponses({
															...
															@RequestParam(value = Constants.PAGESIZE, defaultValue = Constants.defaultGroupByPageSize+"", required = false) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new RelOfertaConvoca(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,RelOfertaConvocaSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=RelOfertaConvocaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, RelOfertaConvocaSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<RelOfertaConvoca>, EntityManager> visitor = new JpaCriteriaQueryVisitor<RelOfertaConvoca>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new RelOfertaConvoca(), new RelOfertaConvocaResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, RelOfertaConvocaSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) String rsqlQ, 
			@RequestParam(value = Constants.PAGE, defaultValue = "1", required = false) String page, 
			@RequestParam(value = Constants.PAGESIZE, defaultValue = "", required = false) String pageSize,
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=RelOfertaConvocaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			...
			@RequestBody RelOfertaConvoca obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=RelOfertaConvocaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=RelOfertaConvocaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new RelOfertaConvoca(), nameController, DELETE, service,getKey());
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=RelOfertaConvocaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable String id)
	{
		...
		return record(request, id, new RelOfertaConvoca(),new RelOfertaConvocaResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_PLANTILLA_TEXT) @RequestBody RelOfertaConvoca obj) {
	...
	private List<String> checkClavesExternas(RelOfertaConvoca objCheck) {
		List<String> errores = new ArrayList<String>();
		// CHECK 1 Convocatorias
		// Chequeamos los identificadores que apuntan a otras tablas
		if (Util.validValue(objCheck.getConvocatoriaId()) && activeFK) {
			ConvocatoriaEmpleoPublico findById = convocatoriaService.findById(getKey(), ConvocatoriaEmpleoPublico.class,
					objCheck.getConvocatoriaId());
			if (findById == null) {
				errores.add("ConvocatoriaEmpleoPublico ID not exists in entity: " + objCheck.getConvocatoriaId());
			}
		}

		// CHECK 2 Oferta
		// Chequeamos los identificadores que apuntan a otras tablas
		if (Util.validValue(objCheck.getOfertaId()) && activeFK) {
		  OfertaEmpleoPublico findById = ofertaService.findById(getKey(), OfertaEmpleoPublico.class, objCheck.getOfertaId());
			if (findById == null) {
				errores.add("OfertaPublica ID not exists in entity: " + objCheck.getOfertaId());
			}
		}
