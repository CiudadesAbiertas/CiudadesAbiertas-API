...
import org.ciudadesabiertas.dataset.utils.ConvocatoriaEmpleoPublicoResult;
import org.ciudadesabiertas.dataset.utils.ConvocatoriaEmpleoPublicoSearch;
import org.ciudadesabiertas.dataset.utils.EmpleoConstants;
import org.ciudadesabiertas.dataset.utils.PlazaPorTurnoSearch;
...
@RestController
@Api(value="Convocatoria Publica",description = "Conjunto de operaciones relacionadas con el conjunto de datos Convocatoria de Empleo Público en Empleo"+SwaggerConstants.VOCABULARIO_A_HREF+EmpleoConstants.vocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END , tags= {"Empleo - Convocatoria de Empleo Público"})
public class ConvocatoriaEmpleoPublicoController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/empleo/convocatoria-empleo-publico";
	...
	public static final String MODEL_VIEW_LIST = "empleo/convocatoriaPublica/list";
	public static final String MODEL_VIEW_ID = "empleo/convocatoriaPublica/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = ConvocatoriaEmpleoPublicoController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("ConvocatoriaPublica_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("ConvocatoriaPublica_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("ConvocatoriaPublica_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("ConvocatoriaPublica_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("ConvocatoriaPublica_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("ConvocatoriaPublica_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(ConvocatoriaEmpleoPublico.class);

	private static final Logger log = LoggerFactory.getLogger(ConvocatoriaEmpleoPublicoController.class);
		

	@Autowired
	protected DatasetService<ConvocatoriaEmpleoPublico> service;
	
	@Autowired
	protected DatasetService<PlazaPorTurno> plazaPorTurnoService;
	
	@Autowired
	protected DatasetService<RelOfertaConvoca> relOfertaConvoService;
															...
															@RequestParam(value = Constants.PAGESIZE, defaultValue = Constants.defaultGroupByPageSize+"", required = false) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new ConvocatoriaEmpleoPublico(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,ConvocatoriaEmpleoPublicoSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=ConvocatoriaEmpleoPublicoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, ConvocatoriaEmpleoPublicoSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<ConvocatoriaEmpleoPublico>, EntityManager> visitor = new JpaCriteriaQueryVisitor<ConvocatoriaEmpleoPublico>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new ConvocatoriaEmpleoPublico(), new ConvocatoriaEmpleoPublicoResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, ConvocatoriaEmpleoPublicoSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) String rsqlQ, 
			@RequestParam(value = Constants.PAGE, defaultValue = "1", required = false) String page, 
			@RequestParam(value = Constants.PAGESIZE, defaultValue = "", required = false) String pageSize,
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=ConvocatoriaEmpleoPublicoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			...
			@RequestBody ConvocatoriaEmpleoPublico obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=ConvocatoriaEmpleoPublicoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=ConvocatoriaEmpleoPublicoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new ConvocatoriaEmpleoPublico(), nameController, DELETE, service,getKey(), erroresFK);
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=ConvocatoriaEmpleoPublicoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable String id)
	{
		...
		return record(request, id, new ConvocatoriaEmpleoPublico(),new ConvocatoriaEmpleoPublicoResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_PLANTILLA_TEXT) @RequestBody ConvocatoriaEmpleoPublico obj) {
	...
	private List<String> checkClavesExternasRowCount(String id)
	{
		...
		if (Util.validValue(id) && activeFK)
		{
			try {
    			//1 CHECK
    			PlazaPorTurnoSearch plazaPorTurnoSearch = new PlazaPorTurnoSearch();
    			plazaPorTurnoSearch.setConvocatoriaId(id);
    			long rowcount = plazaPorTurnoService.rowcount(getKey(), PlazaPorTurno.class, plazaPorTurnoSearch);
    			if (rowcount>0) {
    				errores.add("This Convocatoria is used " + rowcount + " times in PlazaPorTurno");
    			}
    			//2 CHECK
    			RelOfertaConvocaSearch relOfertaConvocaSearch = new RelOfertaConvocaSearch();
    			relOfertaConvocaSearch.setConvocatoriaId(id);
    			rowcount = relOfertaConvoService.rowcount(getKey(), RelOfertaConvoca.class, relOfertaConvocaSearch);
    			if (rowcount>0) {
    				errores.add("This Convocatoria is used " + rowcount + " times in RelOfertaConvoca");
    			}
