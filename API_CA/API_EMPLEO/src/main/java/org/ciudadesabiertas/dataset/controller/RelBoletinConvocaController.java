...
import org.ciudadesabiertas.dataset.utils.RelBoletinConvocaResult;
import org.ciudadesabiertas.dataset.utils.RelBoletinConvocaSearch;
...
public class RelBoletinConvocaController extends GenericController implements CiudadesAbiertasController 
{
	...
	public static final String MODEL_VIEW_LIST = "empleo/relBoletinConvocatoria/list";
	public static final String MODEL_VIEW_ID = "empleo/relBoletinConvocatoria/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = RelBoletinConvocaController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("RelBoletinConvoca_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("RelBoletinConvoca_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("RelBoletinConvoca_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("RelBoletinConvoca_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("RelBoletinConvoca_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("RelBoletinConvoca_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(RelBoletinConvoca.class);

	private static final Logger log = LoggerFactory.getLogger(RelBoletinConvocaController.class);
		

	@Autowired
	protected DatasetService<RelBoletinConvoca> service;
	
	@Autowired
	protected DatasetService<BoletinOficial> serviceBoletin;
	...
	public @ResponseBody ResponseEntity<?> distinctSearch(HttpServletRequest request, DistinctSearch search,															
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new RelBoletinConvoca(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,RelBoletinConvocaSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=RelBoletinConvocaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, RelBoletinConvocaSearch search, 
		...
		RSQLVisitor<CriteriaQuery<RelBoletinConvoca>, EntityManager> visitor = new JpaCriteriaQueryVisitor<RelBoletinConvoca>();
		
		ResponseEntity<RelBoletinConvoca> list= list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new RelBoletinConvoca(), new RelBoletinConvocaResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, RelBoletinConvocaSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields,
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=RelBoletinConvocaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			...
			@RequestBody RelBoletinConvoca obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=RelBoletinConvocaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=RelBoletinConvocaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new RelBoletinConvoca(), nameController, DELETE, service,getKey());
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=RelBoletinConvocaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_CONVENIOSRELFIRAYTO) String id)
	{
		...
		ResponseEntity<RelBoletinConvoca> record= record(request, id, new RelBoletinConvoca(),new RelBoletinConvocaResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_CONVENIOSRELFIRAYTO_TEXT) @RequestBody RelBoletinConvoca obj) {
	...
	private List<String> checkClavesExternas(RelBoletinConvoca objCheck) {
		...
		if (Util.validValue(objCheck.getConvocatoriaId()) && activeFK) {
			ConvocatoriaEmpleoPublico findById = serviceConvocatoria.findById(getKey(), ConvocatoriaEmpleoPublico.class,
		...
		// CHECK 2 Boletines
		// Chequeamos los identificadores que apuntan a otras tablas
		if (Util.validValue(objCheck.getBoletinId()) && activeFK) {
			BoletinOficial findById = serviceBoletin.findById(getKey(), BoletinOficial.class,
					objCheck.getBoletinId());
			if (findById == null) {
				errores.add("BoletinOficial ID not exists in entity: " + objCheck.getBoletinId());
			}
		}
