...
import org.ciudadesabiertas.dataset.utils.CuboNacionalidadResult;
import org.ciudadesabiertas.dataset.utils.CuboNacionalidadSearch;
...
@RestController
@Api(value="Cubo Nacionalidad",description = "Conjunto de operaciones relacionadas con sobre el Cubo de Datos Nacionalidad de Padrón"+SwaggerConstants.VOCABULARIO_A_HREF+CuboPadronConstants.observationVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Padrón - Cubo de datos Nacionalidad"})
public class CuboNacionalidadController<T> extends GenericController implements CiudadesAbiertasController 
{
	...
	public static final String LIST = "/padron/datacube/nacionalidad/observation";	
	public static final String RECORD = "/padron/datacube/nacionalidad/observation/{id}";
	
	public static final String MODEL_VIEW_LIST = "datacube/nacionalidad/list";
	public static final String MODEL_VIEW_ID = "datacube/nacionalidad/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = CuboNacionalidadController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("CUBONACIONALIDAD_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("CUBONACIONALIDAD_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("CUBONACIONALIDAD_QUERY", QUERY, HttpMethod.GET,Constants.NO_AUTH));
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(CuboNacionalidad.class);

	private static final Logger log = LoggerFactory.getLogger(CuboNacionalidadController.class);
	
	@Autowired
	protected DatasetService<CuboNacionalidad> service;
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,CuboNacionalidadSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=CuboNacionalidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, CuboNacionalidadSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<CuboNacionalidad>, EntityManager> visitor = new JpaCriteriaQueryVisitor<CuboNacionalidad>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new CuboNacionalidad(), new CuboNacionalidadResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, CuboNacionalidadSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ,			 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=CuboNacionalidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PADRON_NACIONALIDAD) String id)
	{
		...
		return record(request, id, new CuboNacionalidad(), new CuboNacionalidadResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PADRON_NACIONALIDAD) String id)
	{
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return datacubeSearch(request, cubeQuerySearch, page, pageSize,getKey(), new CuboNacionalidad(), new ObjectResult(), service);
