...
import org.ciudadesabiertas.dataset.utils.CuboEdadGrupoQuinquenalResult;
import org.ciudadesabiertas.dataset.utils.CuboEdadGrupoQuinquenalSearch;
...
@RestController
@Api(value="Cubo Edad por Grupo Quinquenales",description = "Conjunto de operaciones relacionadas con sobre el Cubo de Datos Edad de Padrón"+SwaggerConstants.VOCABULARIO_A_HREF+CuboPadronConstants.observationVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Padrón - Cubo de datos Edad Grupos Quinquenales"})
public class CuboEdadGrupoQuinquenalController<T> extends GenericController implements CiudadesAbiertasController 
{
	public static final String QUERY = "/padron/datacube/edad-grupo-quinquenal/query";	
	public static final String LIST = "/padron/datacube/edad-grupo-quinquenal/observation";	
	public static final String RECORD = "/padron/datacube/edad-grupo-quinquenal/observation/{id}";
	
	public static final String MODEL_VIEW_LIST = "datacube/edadGrupoQuinquenal/list";
	public static final String MODEL_VIEW_ID = "datacube/edadGrupoQuinquenal/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = CuboEdadGrupoQuinquenalController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("CUBOEDAD_G_Q_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("CUBOEDAD_G_Q_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("CUBOEDAD_G_Q_QUERY", QUERY, HttpMethod.GET,Constants.NO_AUTH));
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(CuboEdadGrupoQuinquenal.class);

	private static final Logger log = LoggerFactory.getLogger(CuboEdadGrupoQuinquenalController.class);
	
	@Autowired
	protected DatasetService<CuboEdadGrupoQuinquenal> service;
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,CuboEdadGrupoQuinquenalSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=CuboEdadGrupoQuinquenalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, CuboEdadGrupoQuinquenalSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<CuboEdadGrupoQuinquenal>, EntityManager> visitor = new JpaCriteriaQueryVisitor<CuboEdadGrupoQuinquenal>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new CuboEdadGrupoQuinquenal(), new CuboEdadGrupoQuinquenalResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, CuboEdadGrupoQuinquenalSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=CuboEdadGrupoQuinquenalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PADRON_EDAD_QUINQUENAL) String id)
	{
		...
		return record(request, id, new CuboEdadGrupoQuinquenal(), new CuboEdadGrupoQuinquenalResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PADRON_EDAD_QUINQUENAL) String id)
	{
				...
				@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return datacubeSearch(request, cubeQuerySearch, page, pageSize,getKey(), new CuboEdadGrupoQuinquenal(), new ObjectResult(), service);
