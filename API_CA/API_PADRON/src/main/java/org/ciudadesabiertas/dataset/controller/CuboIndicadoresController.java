...
import org.ciudadesabiertas.dataset.utils.CuboIndicadoresResult;
import org.ciudadesabiertas.dataset.utils.CuboIndicadoresSearch;
...
@RestController
@Api(value="Cubo indicadores demográficos",description = "Conjunto de operaciones relacionadas con sobre el Cubo de Datos Indicadores Demográficos de Padrón"+SwaggerConstants.VOCABULARIO_A_HREF+CuboPadronConstants.observationVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Padrón - Cubo de datos Indicadores Demográficos"})
public class CuboIndicadoresController<T> extends GenericController implements CiudadesAbiertasController 
{
	public static final String QUERY = "/padron/datacube/indicadores/query";	
	public static final String LIST = "/padron/datacube/indicadores/observation";	
	public static final String RECORD = "/padron/datacube/indicadores/observation/{id}";

	public static final String MODEL_VIEW_LIST = "datacube/indicadores/list";
	public static final String MODEL_VIEW_ID = "datacube/indicadores/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = CuboIndicadoresController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("CUBOINDICADORES_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("CUBOINDICADORES_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("CUBOINDICADORES_QUERY", QUERY, HttpMethod.GET,Constants.NO_AUTH));
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(CuboIndicadores.class);

	private static final Logger log = LoggerFactory.getLogger(CuboIndicadoresController.class);
	
	@Autowired
	protected DatasetService<CuboIndicadores> service;
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,CuboIndicadoresSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=CuboIndicadoresResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, CuboIndicadoresSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<CuboIndicadores>, EntityManager> visitor = new JpaCriteriaQueryVisitor<CuboIndicadores>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new CuboIndicadores(), new CuboIndicadoresResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, CuboIndicadoresSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=CuboIndicadoresResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PADRON_INDICADORES) String id)
	{
		...
		return record(request, id, new CuboIndicadores(), new CuboIndicadoresResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PADRON_INDICADORES) String id)
	{
															...
															@RequestParam(value = Constants.PAGESIZE, defaultValue = Constants.defaultGroupByPageSize+"", required = false) String pageSize)
	{
		...
		return datacubeSearch(request, cubeQuerySearch, page, pageSize,getKey(), new CuboIndicadores(), new ObjectResult(), service);
