...
import org.ciudadesabiertas.dataset.utils.CuboEdadResult;
import org.ciudadesabiertas.dataset.utils.CuboEdadSearch;
...
@RestController
@Api(value="Cubo Edad",description = "Conjunto de operaciones relacionadas con sobre el Cubo de Datos Edad de Padrón"+SwaggerConstants.VOCABULARIO_A_HREF+CuboPadronConstants.observationVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Padrón - Cubo de datos Edad"})
public class CuboEdadController<T> extends GenericController implements CiudadesAbiertasController 
{
	public static final String QUERY = "/padron/datacube/edad/query";	
	public static final String LIST = "/padron/datacube/edad/observation";	
	public static final String RECORD = "/padron/datacube/edad/observation/{id}";
	
	public static final String MODEL_VIEW_LIST = "datacube/edad/list";
	public static final String MODEL_VIEW_ID = "datacube/edad/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = CuboEdadController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("CUBOEDAD_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("CUBOEDAD_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("CUBOEDAD_QUERY", QUERY, HttpMethod.GET,Constants.NO_AUTH));
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(CuboEdad.class);

	private static final Logger log = LoggerFactory.getLogger(CuboEdadController.class);
	
	@Autowired
	protected DatasetService<CuboEdad> service;
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,CuboEdadSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=CuboEdadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, CuboEdadSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<CuboEdad>, EntityManager> visitor = new JpaCriteriaQueryVisitor<CuboEdad>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new CuboEdad(), new CuboEdadResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, CuboEdadSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=CuboEdadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PADRON_EDAD) String id)
	{
		...
		return record(request, id, new CuboEdad(), new CuboEdadResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PADRON_EDAD) String id)
	{
				...
				@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return datacubeSearch(request, cubeQuerySearch, page, pageSize,getKey(), new CuboEdad(), new ObjectResult(), service);
