...
import org.ciudadesabiertas.dataset.utils.CuboEstudiosResult;
import org.ciudadesabiertas.dataset.utils.CuboEstudiosSearch;
...
@Api(value="Cubo Estudios",description = "Conjunto de operaciones relacionadas con sobre el Cubo de Datos Nivel de Estudios de Padrón"+SwaggerConstants.VOCABULARIO_A_HREF+CuboPadronConstants.observationVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Padrón - Cubo de datos Estudios"})
public class CuboEstudiosController<T> extends GenericController implements CiudadesAbiertasController 
{
	public static final String QUERY = "/padron/datacube/estudios/query";	
	public static final String LIST = "/padron/datacube/estudios/observation";	
	public static final String RECORD = "/padron/datacube/estudios/observation/{id}";

	
	public static final String MODEL_VIEW_LIST = "datacube/estudios/list";
	public static final String MODEL_VIEW_ID = "datacube/estudios/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = CuboEstudiosController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("CUBOESTUDIO_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("CUBOESTUDIO_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("CUBOESTUDIO_QUERY", QUERY, HttpMethod.GET,Constants.NO_AUTH));
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(CuboEstudios.class);

	private static final Logger log = LoggerFactory.getLogger(CuboEstudiosController.class);
	
	@Autowired
	protected DatasetService<CuboEstudios> service;
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,CuboEstudiosSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=CuboEstudiosResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, CuboEstudiosSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<CuboEstudios>, EntityManager> visitor = new JpaCriteriaQueryVisitor<CuboEstudios>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new CuboEstudios(), new CuboEstudiosResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, CuboEstudiosSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=CuboEstudiosResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PADRON_ESTUDIOS) String id)
	{
		...
		return record(request, id, new CuboEstudios(),new CuboEstudiosResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_PADRON_ESTUDIOS) String id)
	{
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return datacubeSearch(request, cubeQuerySearch, page, pageSize,getKey(), new CuboEstudios(), new ObjectResult(), service);
