...
import org.ciudadesabiertas.dataset.utils.DeudaFinancieraRelPrestamoEntidadResult;
...
@RestController
@Api(value="Relación entre Prestamo y Entidad en Deuda Financiera",description = "Conjunto de operaciones relacionadas con el conjunto de datos Deuda Financiera"+SwaggerConstants.VOCABULARIO_A_HREF+DeudaConstants.deudaFinancieraVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Deuda Finaciera -  Relación Prestamo Entidad"})
public class DeudaFinancieraRelPrestamoEntidadController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/deuda-publica-financiera/rel-prestamo-entidad";
	...
	public static final String MODEL_VIEW_LIST = "deuda/financiera/relPrestamoEnt/list";
	public static final String MODEL_VIEW_ID = "deuda/financiera/relPrestamoEnt/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = DeudaFinancieraRelPrestamoEntidadController.class.getName();
	
	
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("DeudaFinancieraRelPrestamoEntidad_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraRelPrestamoEntidad_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraRelPrestamoEntidad_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("DeudaFinancieraRelPrestamoEntidad_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraRelPrestamoEntidad_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraRelPrestamoEntidad_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(DeudaFinancieraRelPrestamoEntidad.class);

	private static final Logger log = LoggerFactory.getLogger(DeudaFinancieraRelPrestamoEntidadController.class);
		

	@Autowired
	protected DatasetService<DeudaFinancieraRelPrestamoEntidad> service;
	
	@Autowired
	protected DatasetService<DeudaFinancieraPrestamo> servicePrestamo;
	
	@Autowired
	protected DatasetService<DeudaOrganization> serviceOrganization;
	    		...
	    		@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new DeudaFinancieraRelPrestamoEntidad(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,DeudaFinancieraRelPrestamoEntidadSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=DeudaFinancieraRelPrestamoEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, DeudaFinancieraRelPrestamoEntidadSearch search, 
		...
		@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<DeudaFinancieraRelPrestamoEntidad>, EntityManager> visitor = new JpaCriteriaQueryVisitor<DeudaFinancieraRelPrestamoEntidad>();
		ResponseEntity<DeudaFinancieraRelPrestamoEntidad> list= list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new DeudaFinancieraRelPrestamoEntidad(), new DeudaFinancieraRelPrestamoEntidadResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, DeudaFinancieraRelPrestamoEntidadSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields,
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=DeudaFinancieraRelPrestamoEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_DEUDA_F_REL_PRESTAMO_ENT_TEXT) 			
			@RequestBody DeudaFinancieraRelPrestamoEntidad obj 
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getPrestamoId(), obj.getEntidadPrestamista(), Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=DeudaFinancieraRelPrestamoEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_DEUDA_F_REL_PRESTAMO_ENT) 
			...
			@RequestBody DeudaFinancieraRelPrestamoEntidad obj)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getPrestamoId(), obj.getEntidadPrestamista(), Constants.BASIC_OPERATION_UPDATE);
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=DeudaFinancieraRelPrestamoEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new DeudaFinancieraRelPrestamoEntidad(), nameController, DELETE, service,getKey());
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=DeudaFinancieraRelPrestamoEntidadResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, 
			@PathVariable 
			@ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_DEUDA_F_REL_PRESTAMO_ENT) String id)
	{
		...
		ResponseEntity<DeudaFinancieraRelPrestamoEntidad> record = record(request, id, new DeudaFinancieraRelPrestamoEntidad(),new DeudaFinancieraRelPrestamoEntidadResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_DEUDA_F_REL_PRESTAMO_ENT_TEXT) @RequestBody DeudaFinancieraRelPrestamoEntidad obj) {
	...
	private List<String> checkClavesExternas(String prestamoId, String entidadId, String operation) {
		
		List<String> errors=new ArrayList<String>();
		
		log.debug("checkClavesExternas] [prestamoId:"+prestamoId+"] [entidadId:"+entidadId+"] [operation:"+operation+"]");
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_ADD)||(operation.equals(Constants.BASIC_OPERATION_UPDATE)))
			{
				if (prestamoId!=null)
				{
					...
					//1 FK DeudaFinancieraPrestamo
					DeudaOrganizationSearch fk2Search=new DeudaOrganizationSearch();
					fk2Search.setId(entidadId);
					long rowcount2 = serviceOrganization.rowcount(getKey(), DeudaOrganization.class, fk2Search);
					if (rowcount2==0)			
					{
						errors.add("The DeudaOrganization '"+entidadId+"' No Existe");		
					}
				}
			}			
