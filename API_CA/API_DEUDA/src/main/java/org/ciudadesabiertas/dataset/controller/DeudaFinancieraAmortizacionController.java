...
import org.ciudadesabiertas.dataset.utils.DeudaFinancieraAmortizacionResult;
...
@RestController
@Api(value="Plan de Amortización de Deuda Financiera",description = "Conjunto de operaciones relacionadas con el conjunto de datos Deuda Financiera"+SwaggerConstants.VOCABULARIO_A_HREF+DeudaConstants.deudaFinancieraVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Deuda Finaciera -  Plan de Amortización"})
public class DeudaFinancieraAmortizacionController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/deuda-publica-financiera/amortizacion";
	...
	public static final String MODEL_VIEW_LIST = "deuda/financiera/amortizacion/list";
	public static final String MODEL_VIEW_ID = "deuda/financiera/amortizacion/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = DeudaFinancieraAmortizacionController.class.getName();
	
	
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("DeudaFinancieraAmortizacion_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraAmortizacion_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraAmortizacion_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("DeudaFinancieraAmortizacion_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraAmortizacion_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraAmortizacion_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(DeudaFinancieraAmortizacion.class);

	private static final Logger log = LoggerFactory.getLogger(DeudaFinancieraAmortizacionController.class);
		

	@Autowired
	protected DatasetService<DeudaFinancieraAmortizacion> service;
	
	
	@Autowired
	protected DatasetService<DeudaFinancieraInstrumentoFinanciacion> serviceInstrumentaF;
	    		...
	    		@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new DeudaFinancieraAmortizacion(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,DeudaFinancieraAmortizacionSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=DeudaFinancieraAmortizacionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, DeudaFinancieraAmortizacionSearch search, 
		...
		@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<DeudaFinancieraAmortizacion>, EntityManager> visitor = new JpaCriteriaQueryVisitor<DeudaFinancieraAmortizacion>();
		ResponseEntity<DeudaFinancieraAmortizacion> list= list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new DeudaFinancieraAmortizacion(), new DeudaFinancieraAmortizacionResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, DeudaFinancieraAmortizacionSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields,
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=DeudaFinancieraAmortizacionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_DEUDA_F_AMORTIZACION_TEXT) 			
			@RequestBody DeudaFinancieraAmortizacion obj 
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getInstrumentoFinanciacion(), Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=DeudaFinancieraAmortizacionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_DEUDA_F_AMORTIZACION) 
			...
			@RequestBody DeudaFinancieraAmortizacion obj)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getInstrumentoFinanciacion(), Constants.BASIC_OPERATION_UPDATE);
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=DeudaFinancieraAmortizacionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new DeudaFinancieraAmortizacion(), nameController, DELETE, service,getKey());
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=DeudaFinancieraAmortizacionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, 
			@PathVariable 
			@ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_DEUDA_F_AMORTIZACION) String id)
	{
		...
		ResponseEntity<DeudaFinancieraAmortizacion> record = record(request, id, new DeudaFinancieraAmortizacion(),new DeudaFinancieraAmortizacionResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_DEUDA_F_AMORTIZACION_TEXT) @RequestBody DeudaFinancieraAmortizacion obj) {
	...
	private List<String> checkClavesExternas(String instrumentoFinanciacionId, String operation) {
		
		List<String> errors=new ArrayList<String>();
		
		log.debug("checkClavesExternas] [instrumentoFinanciacionId:"+instrumentoFinanciacionId+"] [operation:"+operation+"]");
		...
		{	
			...
			{
				if (instrumentoFinanciacionId!=null)
				{
					//1 FK DeudaFinancieraInstrumentoFinanciacion
					DeudaFinancieraInstrumentoFinanciacionSearch fk1Search=new DeudaFinancieraInstrumentoFinanciacionSearch();
					fk1Search.setId(instrumentoFinanciacionId);
					long rowcount = serviceInstrumentaF.rowcount(getKey(), DeudaFinancieraInstrumentoFinanciacion.class, fk1Search);
					if (rowcount==0)			
					{
						errors.add("The InstrumentoFinanciacion '"+instrumentoFinanciacionId+"' No Existe");		
					}
				}
			}			
