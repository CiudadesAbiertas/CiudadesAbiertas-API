...
import org.ciudadesabiertas.dataset.model.DeudaComercialInformePMPMensualGlobal;
...
import org.ciudadesabiertas.dataset.utils.DeudaComercialInformePMPMensualGlobalSearch;
import org.ciudadesabiertas.dataset.utils.DeudaComercialInformePMPMensualSearch;
import org.ciudadesabiertas.dataset.utils.DeudaComercialProperIntervalResult;
...
@RestController
@Api(value="Intervalos de Deuda Comercial",description = "Conjunto de operaciones relacionadas con el conjunto de datos Intervalo"+SwaggerConstants.VOCABULARIO_A_HREF+DeudaConstants.deudaComercialVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Deuda Comercial - Intervalos de tiempo"})
public class DeudaComercialProperIntervalController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/deuda-publica-comercial/proper-interval";
	...
	public static final String MODEL_VIEW_LIST = "deuda/comercial/properInterval/list";
	public static final String MODEL_VIEW_ID = "deuda/comercial/properInterval/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = DeudaComercialProperIntervalController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("DeudaComercialProperInterval_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DeudaComercialProperInterval_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DeudaComercialProperInterval_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("DeudaComercialProperInterval_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DeudaComercialProperInterval_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DeudaComercialProperInterval_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(DeudaComercialProperInterval.class);
	{
		availableFields.add(Constants.XETRS89Fin);
		availableFields.add(Constants.YETRS89Fin);
	}

	private static final Logger log = LoggerFactory.getLogger(DeudaComercialProperIntervalController.class);
		

	@Autowired
	protected DatasetService<DeudaComercialProperInterval> service;
	...
	@Autowired
	protected DatasetService<DeudaComercialInformePMPMensualGlobal> serviceInformePMPMensualGlobal;
				...
				@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new DeudaComercialProperInterval(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,DeudaComercialProperIntervalSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=DeudaComercialProperIntervalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, DeudaComercialProperIntervalSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<DeudaComercialProperInterval>, EntityManager> visitor = new JpaCriteriaQueryVisitor<DeudaComercialProperInterval>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new DeudaComercialProperInterval(), new DeudaComercialProperIntervalResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, DeudaComercialProperIntervalSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false)
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false)
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=DeudaComercialProperIntervalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			...
			@RequestBody DeudaComercialProperInterval obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=DeudaComercialProperIntervalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_TEXT+SwaggerConstants.PARAM_ID_DEUDA_C_PROPER_INTERVAL) 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=DeudaComercialProperIntervalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new DeudaComercialProperInterval(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=DeudaComercialProperIntervalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_DEUDA_C_PROPER_INTERVAL) String id)
	{
		...
		return record(request, id, new DeudaComercialProperInterval(),new DeudaComercialProperIntervalResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_DEUDA_C_PROPER_INTERVAL) String id)
	{
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_TRAFICOPORPINTE_TEXT) @RequestBody DeudaComercialProperInterval obj) {
	...
	private List<String> checkClavesExternas(String id) {
		...
		{		
			...
			//3 FK InformePMPMensualGlobal
			DeudaComercialInformePMPMensualGlobalSearch fk3Search=new DeudaComercialInformePMPMensualGlobalSearch();
			fk3Search.setPeriodo(id);
			rowcount = serviceInformePMPMensualGlobal.rowcount(getKey(), DeudaComercialInformePMPMensualGlobal.class, fk3Search);
			if (rowcount>0)			
			{
				errors.add("The organization '"+id+"' is used in "+ rowcount+" DeudaComercialInformePMPMensualGlobal");		
			}
		...
		{
			log.error("[checkClavesExternas] [interval:"+id+"] [ERROR:" + e.getMessage() + "]");
			errors.add("[checkClavesExternas] [interval:"+id+"] [ERROR:" + e.getMessage() + "]");
		}
