...
import org.ciudadesabiertas.dataset.utils.DeudaFinancieraPrestamoResult;
...
@RestController
@Api(value="Prestamo de Deuda Financiera",description = "Conjunto de operaciones relacionadas con el conjunto de datos Deuda Financiera"+SwaggerConstants.VOCABULARIO_A_HREF+DeudaConstants.deudaFinancieraVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Deuda Finaciera -  Prestamo"})
public class DeudaFinancieraPrestamoController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/deuda-publica-financiera/prestamo";
	...
	public static final String MODEL_VIEW_LIST = "deuda/financiera/prestamo/list";
	public static final String MODEL_VIEW_ID = "deuda/financiera/prestamo/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = DeudaFinancieraPrestamoController.class.getName();
	
	
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("DeudaFinancieraPrestamo_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraPrestamo_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraPrestamo_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("DeudaFinancieraPrestamo_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraPrestamo_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraPrestamo_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(DeudaFinancieraPrestamo.class);

	private static final Logger log = LoggerFactory.getLogger(DeudaFinancieraPrestamoController.class);
		

	@Autowired
	protected DatasetService<DeudaFinancieraPrestamo> service;
	
	@Autowired
	protected DatasetService<DeudaFinancieraInstrumentoFinanciacion> serviceInstrumentaF;
	
	@Autowired
	protected DatasetService<DeudaFinancieraRelPrestamoEntidad> serviceRelPrestamoEnt;
	    		...
	    		@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new DeudaFinancieraPrestamo(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,DeudaFinancieraPrestamoSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=DeudaFinancieraPrestamoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, DeudaFinancieraPrestamoSearch search, 
		...
		@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<DeudaFinancieraPrestamo>, EntityManager> visitor = new JpaCriteriaQueryVisitor<DeudaFinancieraPrestamo>();
		ResponseEntity<DeudaFinancieraPrestamo> list= list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new DeudaFinancieraPrestamo(), new DeudaFinancieraPrestamoResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, DeudaFinancieraPrestamoSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields,
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=DeudaFinancieraPrestamoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_DEUDA_F_PRESTAMO_TEXT) 			
			@RequestBody DeudaFinancieraPrestamo obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=DeudaFinancieraPrestamoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_DEUDA_F_PRESTAMO) 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=DeudaFinancieraPrestamoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new DeudaFinancieraPrestamo(), nameController, DELETE, service,getKey(),erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=DeudaFinancieraPrestamoResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, 
			@PathVariable 
			@ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_DEUDA_F_PRESTAMO) String id)
	{
		...
		ResponseEntity<DeudaFinancieraPrestamo> record = record(request, id, new DeudaFinancieraPrestamo(),new DeudaFinancieraPrestamoResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_DEUDA_F_PRESTAMO_TEXT) @RequestBody DeudaFinancieraPrestamo obj) {
	...
	private List<String> checkClavesExternas(String id) {
		...
		log.debug("checkClavesExternas] [id:"+id+"]");		
		...
		try
		{		
			//1 FK InstrumentoFinanciacion
			DeudaFinancieraInstrumentoFinanciacionSearch fk1Search=new DeudaFinancieraInstrumentoFinanciacionSearch();
			fk1Search.setPrestamo(id);
			long rowcount = serviceInstrumentaF.rowcount(getKey(), DeudaFinancieraInstrumentoFinanciacion.class, fk1Search);
			if (rowcount>0)			
			{
				errors.add("The DeudaFinancieraPrestamo '"+id+"' is used in "+rowcount+" DeudaFinancieraInstrumentoFinanciacion");		
			}
			
			//2 FK RelPrestamoEntidad
			DeudaFinancieraRelPrestamoEntidadSearch fk2Search=new DeudaFinancieraRelPrestamoEntidadSearch();
			fk2Search.setPrestamoId(id);
			long rowcount2 = serviceRelPrestamoEnt.rowcount(getKey(), DeudaFinancieraRelPrestamoEntidad.class, fk2Search);
			if (rowcount2>0)			
			{
				errors.add("The DeudaFinancieraPrestamo '"+id+"' is used in "+rowcount+" DeudaFinancieraRelPrestamoEntidad");		
			}
