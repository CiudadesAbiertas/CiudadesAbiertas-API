...
import org.ciudadesabiertas.dataset.utils.DeudaFinancieraEmisionResult;
...
@RestController
@Api(value="Plan de Emisión Financiera",description = "Conjunto de operaciones relacionadas con el conjunto de datos Deuda Financiera"+SwaggerConstants.VOCABULARIO_A_HREF+DeudaConstants.deudaFinancieraVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Deuda Finaciera -  Emisión"})
public class DeudaFinancieraEmisionController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/deuda-publica-financiera/emision";
	...
	public static final String MODEL_VIEW_LIST = "deuda/financiera/emision/list";
	public static final String MODEL_VIEW_ID = "deuda/financiera/emision/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = DeudaFinancieraEmisionController.class.getName();
	
	
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("DeudaFinancieraEmision_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraEmision_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraEmision_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("DeudaFinancieraEmision_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraEmision_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DeudaFinancieraEmision_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(DeudaFinancieraEmision.class);

	private static final Logger log = LoggerFactory.getLogger(DeudaFinancieraEmisionController.class);
		

	@Autowired
	protected DatasetService<DeudaFinancieraEmision> service;
	    		...
	    		@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new DeudaFinancieraEmision(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,DeudaFinancieraEmisionSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=DeudaFinancieraEmisionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, DeudaFinancieraEmisionSearch search, 
		...
		@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<DeudaFinancieraEmision>, EntityManager> visitor = new JpaCriteriaQueryVisitor<DeudaFinancieraEmision>();
		ResponseEntity<DeudaFinancieraEmision> list= list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new DeudaFinancieraEmision(), new DeudaFinancieraEmisionResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, DeudaFinancieraEmisionSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields,
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=DeudaFinancieraEmisionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_DEUDA_F_EMISION_TEXT) 			
			@RequestBody DeudaFinancieraEmision obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=DeudaFinancieraEmisionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_DEUDA_F_EMISION) 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=DeudaFinancieraEmisionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new DeudaFinancieraEmision(), nameController, DELETE, service,getKey(),erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=DeudaFinancieraEmisionResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, 
			@PathVariable 
			@ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_DEUDA_F_EMISION) String id)
	{
		...
		ResponseEntity<DeudaFinancieraEmision> record = record(request, id, new DeudaFinancieraEmision(),new DeudaFinancieraEmisionResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_DEUDA_F_EMISION_TEXT) @RequestBody DeudaFinancieraEmision obj) {
	...
	private List<String> checkClavesExternas(String id) {
		...
		{		
			...
			fk1Search.setEmision(id);
			long rowcount = serviceInstrumentaF.rowcount(getKey(), DeudaFinancieraInstrumentoFinanciacion.class, fk1Search);
			if (rowcount>0)			
			{
				errors.add("The DeudaFinancieraEmision '"+id+"' is used in "+rowcount+" DeudaFinancieraInstrumentoFinanciacion");		
			}
