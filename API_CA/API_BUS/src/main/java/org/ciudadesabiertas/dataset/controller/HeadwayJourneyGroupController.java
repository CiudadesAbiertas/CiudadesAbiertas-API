...
import org.ciudadesabiertas.dataset.utils.HeadwayIntervalSearch;
import org.ciudadesabiertas.dataset.utils.HeadwayJourneyGroupResult;
...
@RestController
@Api(value="HeadwayJourneyGroup",description = "Conjunto de operaciones relacionadas con el conjunto de datos Grupo de viajes en cabecera"+SwaggerConstants.VOCABULARIO_A_HREF+BusConstants.BusVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Autobús - Grupo de viajes en cabecera"})
public class HeadwayJourneyGroupController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/autobus/headwayjourneygroup";
	...
	public static final String MODEL_VIEW_LIST = "autobus/headwayjourneygroup/list";
	public static final String MODEL_VIEW_ID = "autobus/headwayjourneygroup/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = HeadwayJourneyGroupController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("HeadwayJourneyGroup_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("HeadwayJourneyGroup_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("HeadwayJourneyGroup_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("HeadwayJourneyGroup_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("HeadwayJourneyGroup_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("HeadwayJourneyGroup_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(HeadwayJourneyGroup.class);

	private static final Logger log = LoggerFactory.getLogger(HeadwayJourneyGroupController.class);
		

	@Autowired
	protected DatasetService<HeadwayJourneyGroup> service;
	@Autowired
	protected DatasetService<HeadwayInterval> headwayIntervalService;
	@Autowired
															...
															@RequestParam(value = Constants.PAGESIZE, defaultValue = Constants.defaultGroupByPageSize+"", required = false) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new HeadwayJourneyGroup(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,HeadwayJourneyGroupSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=HeadwayJourneyGroupResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, HeadwayJourneyGroupSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<HeadwayJourneyGroup>, EntityManager> visitor = new JpaCriteriaQueryVisitor<HeadwayJourneyGroup>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new HeadwayJourneyGroup(), new HeadwayJourneyGroupResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, HeadwayJourneyGroupSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) String rsqlQ, 
			@RequestParam(value = Constants.PAGE, defaultValue = "1", required = false) String page, 
			@RequestParam(value = Constants.PAGESIZE, defaultValue = "", required = false) String pageSize,
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=HeadwayJourneyGroupResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			...
			@RequestBody HeadwayJourneyGroup obj 
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getDeterminedBy(), null, Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=HeadwayJourneyGroupResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@RequestBody HeadwayJourneyGroup obj)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getDeterminedBy(), null, Constants.BASIC_OPERATION_UPDATE);
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=HeadwayJourneyGroupResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new HeadwayJourneyGroup(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=HeadwayJourneyGroupResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable String id)
	{
		...
		return record(request, id, new HeadwayJourneyGroup(),new HeadwayJourneyGroupResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_PLANTILLA_TEXT) @RequestBody HeadwayJourneyGroup obj) {
	...
	private List<String> checkClavesExternas(String headwayIntervalId, String headwayJourneyGroupId, String operation) {
		...
		{	
			...
			{
				if (headwayIntervalId!=null)
				{
					HeadwayIntervalSearch search=new HeadwayIntervalSearch();
					search.setId(headwayIntervalId);
					long rowcount = headwayIntervalService.rowcount(getKey(), HeadwayInterval.class, search);
					if (rowcount==0)			
					{
						errors.add("El intervalo de cabecera '"+headwayIntervalId+"' no existe");		
					}
				}
			...
			else if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				...
				{					
					...
					search2.setComposedOf(headwayJourneyGroupId);
					long rowcount = vehicleJourneyService.rowcount(getKey(), VehicleJourney.class, search2);
					

					if (rowcount>0)			
					{
						errors.add("El grupo de viajes en cabecera'"+headwayJourneyGroupId+"' está siendo usado en la relación con planificación del vehículo");
					}
