...
import org.ciudadesabiertas.dataset.utils.ScheduledStopPointSearch;
import org.ciudadesabiertas.dataset.utils.StopPointInJourneyPatternResult;
...
@RestController
@Api(value="Punto de Parada en Patrón de Viaje",description = "Conjunto de operaciones relacionadas con el conjunto de datos Punto de Parada en Patrón de Viaje"+SwaggerConstants.VOCABULARIO_A_HREF+BusConstants.BusVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Autobús - Punto de Parada en Patrón de Viaje"})
public class StopPointInJourneyPatternController extends GenericController implements CiudadesAbiertasController
{
	public static final String LIST = "/autobus/stoppointinjourneypattern";	
	...
	public static final String MODEL_VIEW_LIST = "autobus/stoppointinjourneypattern/list";
	public static final String MODEL_VIEW_ID = "autobus/stoppointinjourneypattern/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = StopPointInJourneyPatternController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("StopPointInJourneyPattern_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("StopPointInJourneyPattern_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("StopPointInJourneyPattern_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("StopPointInJourneyPattern_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("StopPointInJourneyPattern_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("StopPointInJourneyPattern_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(StopPointInJourneyPattern.class);

	private static final Logger log = LoggerFactory.getLogger(StopPointInJourneyPatternController.class);	

	@Autowired
	protected DatasetService<StopPointInJourneyPattern> service;
	@Autowired
	protected DatasetService<ScheduledStopPoint> scheduledStopPointService;
	@Autowired
	protected DatasetService<RealTimePassingTime> realTimePassingTimeService;

	/**
	 * Ejemplos de Llamadas a utilizar:
	 * http://localhost:8080/API/StopPointInJourneyPattern.html
	 * http://localhost:8080/API/StopPointInJourneyPattern.html?q=importe>11701&sort=-importe	
	 * http://localhost:8080/API/StopPointInJourneyPattern?q=importe>11701&sort=-importe&fields=nombre,importe
	 * http://localhost:8080/API/StopPointInJourneyPattern?fechaAdjudicacion=2016-03-29T00:00:00
	 * @param mv
	 * @param request
	 * @param search
	 * @param rsqlQ
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,StopPointInJourneyPatternSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=StopPointInJourneyPatternResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, StopPointInJourneyPatternSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<StopPointInJourneyPattern>, EntityManager> visitor = new JpaCriteriaQueryVisitor<StopPointInJourneyPattern>();
		
		ResponseEntity<StopPointInJourneyPattern> list=list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new StopPointInJourneyPattern(), new StopPointInJourneyPatternResult(),	availableFields, getKey(), visitor,service);
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, StopPointInJourneyPatternSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=StopPointInJourneyPatternResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> add(			
			...
			@RequestBody StopPointInJourneyPattern obj 
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getViewedAs(), null, Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=StopPointInJourneyPatternResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@RequestBody StopPointInJourneyPattern obj)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getViewedAs(), null, Constants.BASIC_OPERATION_UPDATE);
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=StopPointInJourneyPatternResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		List<String> erroresFK = checkClavesExternas(null, id, Constants.BASIC_OPERATION_DELETE);
		
		return delete(id, new StopPointInJourneyPattern(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=StopPointInJourneyPatternResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_BUS_STOP_POINT_IN_JOURNEY_PATTERN) String id)
	{
		...
		ResponseEntity<StopPointInJourneyPattern> record=record(request, id, new StopPointInJourneyPattern(), new StopPointInJourneyPatternResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_BUS_STOP_POINT_IN_JOURNEY_PATTERN_TEXT) @RequestBody StopPointInJourneyPattern obj) {
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new StopPointInJourneyPattern(), new ObjectResult(),  service);
	...
	private List<String> checkClavesExternas(String scheduledstoppointId, String stoppointinjourneypatternId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_ADD)||(operation.equals(Constants.BASIC_OPERATION_UPDATE)))
			{
				...
				{
					ScheduledStopPointSearch search=new ScheduledStopPointSearch();
					search.setId(scheduledstoppointId);
					long rowcount = scheduledStopPointService.rowcount(getKey(), ScheduledStopPoint.class, search);
					if (rowcount==0)			
					{
						errors.add("La parada planificada '"+scheduledstoppointId+"' no existe");		
					}
				}
			...
			else if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				...
				{					
					...
					RealTimePassingTimeSearch search=new RealTimePassingTimeSearch();
					search.setHasFeatureOfInterest(stoppointinjourneypatternId);
					long rowcount = realTimePassingTimeService.rowcount(getKey(), RealTimePassingTime.class, search);
					

					if (rowcount>0)			
					{
						errors.add("El punto de Parada en Patrón de Viaje'"+stoppointinjourneypatternId+"' está siendo usado en la relación con tiempo de llegada previsto");
					}
