...
import org.ciudadesabiertas.dataset.utils.HeadwayIntervalResult;
...
@RestController
@Api(value="HeadwayInterval",description = "Conjunto de operaciones relacionadas con el conjunto de datos Intervalo de cabecera"+SwaggerConstants.VOCABULARIO_A_HREF+BusConstants.BusVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Autobús - Intervalo de cabecera"})
public class HeadwayIntervalController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/autobus/headwayinterval";
	...
	public static final String MODEL_VIEW_LIST = "autobus/headwayinterval/list";
	public static final String MODEL_VIEW_ID = "autobus/headwayinterval/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = HeadwayIntervalController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("HeadwayInterval_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("HeadwayInterval_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("HeadwayInterval_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("HeadwayInterval_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("HeadwayInterval_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("HeadwayInterval_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(HeadwayInterval.class);

	private static final Logger log = LoggerFactory.getLogger(HeadwayIntervalController.class);
		

	@Autowired
	protected DatasetService<HeadwayInterval> service;
	@Autowired
															...
															@RequestParam(value = Constants.PAGESIZE, defaultValue = Constants.defaultGroupByPageSize+"", required = false) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new HeadwayInterval(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,HeadwayIntervalSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=HeadwayIntervalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, HeadwayIntervalSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<HeadwayInterval>, EntityManager> visitor = new JpaCriteriaQueryVisitor<HeadwayInterval>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new HeadwayInterval(), new HeadwayIntervalResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, HeadwayIntervalSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) String rsqlQ, 
			@RequestParam(value = Constants.PAGE, defaultValue = "1", required = false) String page, 
			@RequestParam(value = Constants.PAGESIZE, defaultValue = "", required = false) String pageSize,
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=HeadwayIntervalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			...
			@RequestBody HeadwayInterval obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=HeadwayIntervalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=HeadwayIntervalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new HeadwayInterval(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=HeadwayIntervalResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable String id)
	{
		...
		return record(request, id, new HeadwayInterval(),new HeadwayIntervalResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_PLANTILLA_TEXT) @RequestBody HeadwayInterval obj) {
	...
	private List<String> checkClavesExternas(String headwayIntervalId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				if (headwayIntervalId!=null)
				{
					...
					search.setDeterminedBy(headwayIntervalId);
					long rowcount = headwayJourneyGroupService.rowcount(getKey(), HeadwayJourneyGroup.class, search);

					if (rowcount>0)			
					{
						errors.add("El intervalo de cabecera '"+headwayIntervalId+"' está siendo usado en la relación con Grupo de viajes en cabecera");
					}
