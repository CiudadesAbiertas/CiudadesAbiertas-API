...
import org.ciudadesabiertas.dataset.utils.DayTypeAssignmentResult;
import org.ciudadesabiertas.dataset.utils.DayTypeAssignmentSearch;
import org.ciudadesabiertas.dataset.utils.DayTypeSearch;
import org.ciudadesabiertas.dataset.utils.ServiceCalendarSearch;
...
@RestController
@Api(value="DayTypeAssignment",description = "Conjunto de operaciones relacionadas con el conjunto de datos Asignación por día tipo"+SwaggerConstants.VOCABULARIO_A_HREF+BusConstants.BusVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Autobús - Asignación por día tipo"})
public class DayTypeAssignmentController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/autobus/daytypeassignment";
	...
	public static final String MODEL_VIEW_LIST = "autobus/daytypeassignment/list";
	public static final String MODEL_VIEW_ID = "autobus/daytypeassignment/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = DayTypeAssignmentController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("DayTypeAssignment_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DayTypeAssignment_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("DayTypeAssignment_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("DayTypeAssignment_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DayTypeAssignment_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("DayTypeAssignment_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(DayTypeAssignment.class);

	private static final Logger log = LoggerFactory.getLogger(DayTypeAssignmentController.class);
		

	@Autowired
	protected DatasetService<DayTypeAssignment> service;
	@Autowired
	protected DatasetService<DayType> dayTypeService;
	@Autowired
	protected DatasetService<ServiceCalendar> serviceCalendarService;
															...
															@RequestParam(value = Constants.PAGESIZE, defaultValue = Constants.defaultGroupByPageSize+"", required = false) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new DayTypeAssignment(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,DayTypeAssignmentSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=DayTypeAssignmentResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, DayTypeAssignmentSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<DayTypeAssignment>, EntityManager> visitor = new JpaCriteriaQueryVisitor<DayTypeAssignment>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new DayTypeAssignment(), new DayTypeAssignmentResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, DayTypeAssignmentSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) String rsqlQ, 
			@RequestParam(value = Constants.PAGE, defaultValue = "1", required = false) String page, 
			@RequestParam(value = Constants.PAGESIZE, defaultValue = "", required = false) String pageSize,
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=DayTypeAssignmentResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			...
			@RequestBody DayTypeAssignment obj 
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getSpecifying(), obj.getForTheDefinitionOf(), Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=DayTypeAssignmentResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@RequestBody DayTypeAssignment obj)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getSpecifying(), obj.getForTheDefinitionOf(), Constants.BASIC_OPERATION_UPDATE);
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=DayTypeAssignmentResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new DayTypeAssignment(), nameController, DELETE, service,getKey());
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=DayTypeAssignmentResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable String id)
	{
		...
		return record(request, id, new DayTypeAssignment(),new DayTypeAssignmentResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_PLANTILLA_TEXT) @RequestBody DayTypeAssignment obj) {
	...
	private List<String> checkClavesExternas(String dayTypeId, String servCaleId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_ADD)||(operation.equals(Constants.BASIC_OPERATION_UPDATE)))
			{
				if (dayTypeId!=null)
				{
					DayTypeSearch dayTypeSearch=new DayTypeSearch();
					dayTypeSearch.setId(dayTypeId);
					long rowcount = dayTypeService.rowcount(getKey(), DayType.class, dayTypeSearch);
					if (rowcount==0)			
					{
						errors.add("El tipo de día '"+dayTypeId+"' no existe");		
					}
				}
				if (servCaleId!=null)
				{
					ServiceCalendarSearch serviceCalendarSearch=new ServiceCalendarSearch();
					serviceCalendarSearch.setId(servCaleId);
					long rowcount = serviceCalendarService.rowcount(getKey(), ServiceCalendar.class, serviceCalendarSearch);
					if (rowcount==0)			
					{
						errors.add("El servicio de calendario '"+servCaleId+"' no existe");		
					}
				}
