...
import org.ciudadesabiertas.dataset.utils.IncidenciaGeoSearch;
import org.ciudadesabiertas.dataset.utils.IncidenciaResult;
...
@RestController
@Api(value="Incidencia",description = "Conjunto de operaciones relacionadas con el conjunto de datos Incidencia"+SwaggerConstants.VOCABULARIO_A_HREF+BusConstants.BusVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Autobús - Incidencia"})
public class IncidenciaController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/autobus/incidencia";
	...
	public static final String MODEL_VIEW_LIST = "autobus/incidencia/list";
	public static final String MODEL_VIEW_ID = "autobus/incidencia/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = IncidenciaController.class.getName();
	
	@Autowired
	protected DatasetService<Incidencia> service;
	@Autowired
	protected DatasetService<RelLineaIncidencia> relLineaIncidenciaService;
	@Autowired
	protected DatasetService<JourneyPattern> journeyPatternService;
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("Incidencia_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("Incidencia_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("Incidencia_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("Incidencia_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("Incidencia_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("Incidencia_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(Incidencia.class);
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=IncidenciaResult.class),
	...
	public @ResponseBody ResponseEntity<?> geoList(HttpServletRequest request,	IncidenciaGeoSearch search, 
				...
				@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new Incidencia(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,IncidenciaSearch search, 
	...
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, IncidenciaSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false)
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false)
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<Incidencia>, EntityManager> visitor = new JpaCriteriaQueryVisitor<Incidencia>();
		
		ResponseEntity list= list(request, search, fields, rsqlQ, page, pageSize, sort, srId, LIST,new Incidencia(), new IncidenciaResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, IncidenciaSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false)
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false)
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=IncidenciaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_INCIDENCIA_TEXT) 			
			@RequestBody Incidencia obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=IncidenciaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_TEXT+SwaggerConstants.PARAM_ID_BUS_INCIDENCIA) 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=IncidenciaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new Incidencia(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=IncidenciaResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_BUS_INCIDENCIA) String id, @RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value=SwaggerConstants.PARAM_SRID, allowableValues=Constants.SUPPORTED_SRIDS)  String srId)
	{
		...
		ResponseEntity record= record(request, id, new Incidencia(),new IncidenciaResult(), srId, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_BUS_INCIDENCIA) String id, @RequestParam(value = Constants.SRID, defaultValue = Constants.SRID_DEFECTO, required = false) @ApiParam(value=SwaggerConstants.PARAM_SRID, allowableValues=Constants.SUPPORTED_SRIDS)  String srId)
	{
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_INCIDENCIA_TEXT) @RequestBody Incidencia obj) {
	...
	@SuppressWarnings("unchecked")
	private ResponseEntity<?> integraAll(ResponseEntity<Incidencia> list, HttpServletRequest request) {

		if (list != null) {
			// TODO generar todas las integraciones
			 list = (ResponseEntity<Incidencia>) integraTramoTrafico(list,
					request);
			 list = (ResponseEntity<Incidencia>) integraCallejero(list,
					request);
		}
	...
	private List<String> checkClavesExternas(String incidenciaId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				...
				{
					RelLineaIncidenciaSearch search=new RelLineaIncidenciaSearch();
					search.setAfectadaPorIncidencia(incidenciaId);
					long rowcount1 = relLineaIncidenciaService.rowcount(getKey(), RelLineaIncidencia.class, search);
					
					JourneyPatternSearch search2=new JourneyPatternSearch();
					search2.setGeneradoPorIncidencia(incidenciaId);
					long rowcount2 = journeyPatternService.rowcount(getKey(), JourneyPattern.class, search2);
					

					if (rowcount1>0 || rowcount2>0)			
					{
						errors.add("La Incidencia '"+incidenciaId+"' está siendo usado en una relación");
					}
