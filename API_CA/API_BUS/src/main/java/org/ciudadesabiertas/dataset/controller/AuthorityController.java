...
import org.ciudadesabiertas.dataset.utils.AuthorityResult;
import org.ciudadesabiertas.dataset.utils.AuthoritySearch;
...
@RestController
@Api(value="Authority",description = "Conjunto de operaciones relacionadas con el conjunto de datos Autoridad"+SwaggerConstants.VOCABULARIO_A_HREF+BusConstants.BusVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Autobús - Autoridad"})
public class AuthorityController extends GenericController implements CiudadesAbiertasController
{
	public static final String LIST = "/autobus/authority";	
	...
	public static final String MODEL_VIEW_LIST = "autobus/authority/list";
	public static final String MODEL_VIEW_ID = "autobus/authority/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = AuthorityController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("Authority_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("Authority_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("Authority_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("Authority_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("Authority_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("Authority_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(Authority.class);

	private static final Logger log = LoggerFactory.getLogger(AuthorityController.class);	

	@Autowired
	protected DatasetService<Authority> service;
	@Autowired
	protected DatasetService<Operator> operatorService;

	/**
	 * Ejemplos de Llamadas a utilizar:
	 * http://localhost:8080/API/Authority.html
	 * http://localhost:8080/API/Authority.html?q=importe>11701&sort=-importe	
	 * http://localhost:8080/API/Authority?q=importe>11701&sort=-importe&fields=nombre,importe
	 * http://localhost:8080/API/Authority?fechaAdjudicacion=2016-03-29T00:00:00
	 * @param mv
	 * @param request
	 * @param search
	 * @param rsqlQ
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,AuthoritySearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=AuthorityResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, AuthoritySearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<Authority>, EntityManager> visitor = new JpaCriteriaQueryVisitor<Authority>();
		
		ResponseEntity<Authority> list=list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new Authority(), new AuthorityResult(),	availableFields, getKey(), visitor,service);
					
		return (ResponseEntity<Authority>) integraCallejero(list, request);
	}
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, AuthoritySearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=AuthorityResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_BUS_AUTHORITY_TEXT) 			
			@RequestBody Authority obj 
			)
	{
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=AuthorityResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_TEXT+SwaggerConstants.PARAM_ID_BUS_AUTHORITY) 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=AuthorityResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new Authority(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=AuthorityResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_BUS_AUTHORITY) String id)
	{
		...
		ResponseEntity<Authority> record=record(request, id, new Authority(), new AuthorityResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
			
		return (ResponseEntity<Authority>) integraCallejero(record, request);
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_BUS_AUTHORITY) String id)
	{
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_BUS_AUTHORITY_TEXT) @RequestBody Authority obj) {
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new Authority(), new ObjectResult(),  service);
	...
	private List<String> checkClavesExternas(String authorityId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				if (authorityId!=null)
				{
					...
					search.setServingPtFor(authorityId);
					...
					if (rowcount>0)			
					{
						errors.add("La autoridad '"+authorityId+"' está siendo usado en la relación con Operator");
					}
