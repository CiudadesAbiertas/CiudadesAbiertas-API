...
import org.ciudadesabiertas.dataset.utils.HeadwayJourneyGroupSearch;
import org.ciudadesabiertas.dataset.utils.JourneyPatternSearch;
import org.ciudadesabiertas.dataset.utils.VehicleJourneyResult;
...
@RestController
@Api(value="VehicleJourney",description = "Conjunto de operaciones relacionadas con el conjunto de datos Planificación del vehículo"+SwaggerConstants.VOCABULARIO_A_HREF+BusConstants.BusVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Autobús - Planificación del vehículo"})
public class VehicleJourneyController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/autobus/vehiclejourney";
	...
	public static final String MODEL_VIEW_LIST = "autobus/vehiclejourney/list";
	public static final String MODEL_VIEW_ID = "autobus/vehiclejourney/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = VehicleJourneyController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("VehicleJourney_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("VehicleJourney_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("VehicleJourney_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("VehicleJourney_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("VehicleJourney_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("VehicleJourney_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(VehicleJourney.class);

	private static final Logger log = LoggerFactory.getLogger(VehicleJourneyController.class);
		

	@Autowired
	protected DatasetService<VehicleJourney> service;
	@Autowired
	protected DatasetService<JourneyPattern> journeyPatternService;
	@Autowired
	protected DatasetService<DayType> dayTypeService;
	@Autowired
	protected DatasetService<HeadwayJourneyGroup> headwayJourneyGroupService;
															...
															@RequestParam(value = Constants.PAGESIZE, defaultValue = Constants.defaultGroupByPageSize+"", required = false) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new VehicleJourney(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,VehicleJourneySearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=VehicleJourneyResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, VehicleJourneySearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<VehicleJourney>, EntityManager> visitor = new JpaCriteriaQueryVisitor<VehicleJourney>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new VehicleJourney(), new VehicleJourneyResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, VehicleJourneySearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) String rsqlQ, 
			@RequestParam(value = Constants.PAGE, defaultValue = "1", required = false) String page, 
			@RequestParam(value = Constants.PAGESIZE, defaultValue = "", required = false) String pageSize,
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=VehicleJourneyResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> add(			
			...
			@RequestBody VehicleJourney obj 
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getMadeUsing(), obj.getWorkedOn() , obj.getComposedOf(), Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=VehicleJourneyResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@RequestBody VehicleJourney obj)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getMadeUsing(), obj.getWorkedOn() , obj.getComposedOf(), Constants.BASIC_OPERATION_UPDATE);
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=VehicleJourneyResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new VehicleJourney(), nameController, DELETE, service,getKey());
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=VehicleJourneyResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable String id)
	{
		...
		return record(request, id, new VehicleJourney(),new VehicleJourneyResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_PLANTILLA_TEXT) @RequestBody VehicleJourney obj) {
	...
	private List<String> checkClavesExternas(String journeypatternId, String daytypeId, String headwayjourneygroupId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_ADD)||(operation.equals(Constants.BASIC_OPERATION_UPDATE)))
			{
				...
				{
					JourneyPatternSearch search=new JourneyPatternSearch();
					search.setId(journeypatternId);
					long rowcount = journeyPatternService.rowcount(getKey(), JourneyPattern.class, search);
					if (rowcount==0)			
					{
						errors.add("El patrón de viaje '"+journeypatternId+"' no existe");		
					}
				}
				
				if (daytypeId!=null)
				{
					DayTypeSearch search=new DayTypeSearch();
					search.setId(daytypeId);
					long rowcount = dayTypeService.rowcount(getKey(), DayType.class, search);
					if (rowcount==0)			
					{
						errors.add("el día Tipo '"+daytypeId+"' no existe");		
					}
				}
				
				if (headwayjourneygroupId!=null)
				{
					HeadwayJourneyGroupSearch search=new HeadwayJourneyGroupSearch();
					search.setId(headwayjourneygroupId);
					long rowcount = headwayJourneyGroupService.rowcount(getKey(), HeadwayJourneyGroup.class, search);
					if (rowcount==0)			
					{
						errors.add("El grupo de viajes en cabecera '"+headwayjourneygroupId+"' no existe");		
					}
				}
