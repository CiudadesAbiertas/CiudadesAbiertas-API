...
import org.ciudadesabiertas.dataset.utils.IncidenciaSearch;
import org.ciudadesabiertas.dataset.utils.JourneyPatternResult;
import org.ciudadesabiertas.dataset.utils.JourneyPatternSearch;
...
@RestController
@Api(value="JourneyPattern",description = "Conjunto de operaciones relacionadas con el conjunto de datos Patrón de Viaje"+SwaggerConstants.VOCABULARIO_A_HREF+BusConstants.BusVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Autobús - Patrón de viaje"})
public class JourneyPatternController extends GenericController implements CiudadesAbiertasController
{
	public static final String LIST = "/autobus/journeypattern";	
	...
	public static final String MODEL_VIEW_LIST = "autobus/journeypattern/list";
	public static final String MODEL_VIEW_ID = "autobus/journeypattern/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = JourneyPatternController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("Journeypattern_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("Journeypattern_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("Journeypattern_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("Journeypattern_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("Journeypattern_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("Journeypattern_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(JourneyPattern.class);

	private static final Logger log = LoggerFactory.getLogger(JourneyPatternController.class);	

	@Autowired
	protected DatasetService<JourneyPattern> service;
	@Autowired
	protected DatasetService<Route> routeService;
	@Autowired
	protected DatasetService<Incidencia> incidenciaService;
	@Autowired
	protected DatasetService<VehicleJourney> vehicleJourneyService;

	/**
	 * Ejemplos de Llamadas a utilizar:
	 * http://localhost:8080/API/JourneyPattern.html
	 * http://localhost:8080/API/JourneyPattern.html?q=importe>11701&sort=-importe	
	 * http://localhost:8080/API/JourneyPattern?q=importe>11701&sort=-importe&fields=nombre,importe
	 * http://localhost:8080/API/JourneyPattern?fechaAdjudicacion=2016-03-29T00:00:00
	 * @param mv
	 * @param request
	 * @param search
	 * @param rsqlQ
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,JourneyPatternSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=JourneyPatternResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, JourneyPatternSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<JourneyPattern>, EntityManager> visitor = new JpaCriteriaQueryVisitor<JourneyPattern>();
		
		ResponseEntity<JourneyPattern> list=list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new JourneyPattern(), new JourneyPatternResult(),	availableFields, getKey(), visitor,service);
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, JourneyPatternSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=JourneyPatternResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> add(			
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_BUS_JOURNEYPATTERN_TEXT) 			
			@RequestBody JourneyPattern obj 
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getOn(), obj.getGeneradoPorIncidencia() , null, Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=JourneyPatternResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_TEXT+SwaggerConstants.PARAM_ID_BUS_JOURNEYPATTERN) 
			...
			@RequestBody JourneyPattern obj)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getOn(), obj.getGeneradoPorIncidencia() , null, Constants.BASIC_OPERATION_UPDATE);
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=JourneyPatternResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		List<String> erroresFK = checkClavesExternas(null, null, id, Constants.BASIC_OPERATION_DELETE);
		
		return delete(id, new JourneyPattern(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=JourneyPatternResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_BUS_JOURNEYPATTERN) String id)
	{
		...
		ResponseEntity<JourneyPattern> record=record(request, id, new JourneyPattern(), new JourneyPatternResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_BUS_JOURNEYPATTERN) String id)
	{
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_BUS_JOURNEYPATTERN_TEXT) @RequestBody JourneyPattern obj) {
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new JourneyPattern(), new ObjectResult(),  service);
	...
	private List<String> checkClavesExternas(String routeId, String incidenciaId, String journeyPatternId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_ADD)||(operation.equals(Constants.BASIC_OPERATION_UPDATE)))
			{
				if (routeId!=null)
				{
					...
					search.setId(routeId);
					long rowcount = routeService.rowcount(getKey(), Route.class, search);
					if (rowcount==0)			
					{
						errors.add("La ruta '"+routeId+"' no existe");		
					}
				}
				
				if (incidenciaId!=null)
				{
					IncidenciaSearch search=new IncidenciaSearch();
					search.setId(incidenciaId);
					long rowcount = incidenciaService.rowcount(getKey(), Incidencia.class, search);
					if (rowcount==0)			
					{
						errors.add("La incidencia '"+incidenciaId+"' no existe");		
					}
				}
			...
			else if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				...
				{					
					...
					search2.setMadeUsing(journeyPatternId);
					long rowcount = vehicleJourneyService.rowcount(getKey(), VehicleJourney.class, search2);
					

					if (rowcount>0)			
					{
						errors.add("El patrón de viaje'"+journeyPatternId+"' está siendo usado en la relación con planificación del vehículo");
					}
