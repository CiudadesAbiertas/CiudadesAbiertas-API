...
import org.ciudadesabiertas.dataset.utils.ScheduledStopPointResult;
...
@RestController
@Api(value="Parada planificada",description = "Conjunto de operaciones relacionadas con el conjunto de datos Parada planificada"+SwaggerConstants.VOCABULARIO_A_HREF+BusConstants.BusVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Autobús - Parada planificada"})
public class ScheduledStopPointController extends GenericController implements CiudadesAbiertasController
{
	public static final String LIST = "/autobus/scheduledstoppoint";	
	...
	public static final String MODEL_VIEW_LIST = "autobus/scheduledstoppoint/list";
	public static final String MODEL_VIEW_ID = "autobus/scheduledstoppoint/id";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = ScheduledStopPointController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("ScheduledPoint_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("ScheduledPoint_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("ScheduledPoint_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("ScheduledPoint_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("ScheduledPoint_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("ScheduledPoint_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(ScheduledStopPoint.class);

	private static final Logger log = LoggerFactory.getLogger(ScheduledStopPointController.class);	

	@Autowired
	protected DatasetService<ScheduledStopPoint> service;
	@Autowired
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,ScheduledStopPointSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=ScheduledStopPointResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, ScheduledStopPointSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<ScheduledStopPoint>, EntityManager> visitor = new JpaCriteriaQueryVisitor<ScheduledStopPoint>();
		
		ResponseEntity<ScheduledStopPoint> list=list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new ScheduledStopPoint(), new ScheduledStopPointResult(),	availableFields, getKey(), visitor,service);
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, ScheduledStopPointSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=ScheduledStopPointResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	...
	public @ResponseBody ResponseEntity<?> add(			
			...
			@RequestBody ScheduledStopPoint obj 
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getFunctionalCentroidFor(), null, Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=ScheduledStopPointResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@RequestBody ScheduledStopPoint obj)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getFunctionalCentroidFor(), null, Constants.BASIC_OPERATION_UPDATE);
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=ScheduledStopPointResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		return delete(id, new ScheduledStopPoint(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=ScheduledStopPointResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_BUS_STOP_POINT_IN_JOURNEY_PATTERN) String id)
	{
		...
		ResponseEntity<ScheduledStopPoint> record=record(request, id, new ScheduledStopPoint(), new ScheduledStopPointResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_BUS_STOP_POINT_IN_JOURNEY_PATTERN_TEXT) @RequestBody ScheduledStopPoint obj) {
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new ScheduledStopPoint(), new ObjectResult(),  service);
	...
	private List<String> checkClavesExternas(String paradaId, String scheduledstoppointId, String operation) {
		...
		{	
			...
			else if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				...
				{					
					...
					search.setViewedAs(scheduledstoppointId);
					long rowcount = stopPointInJourneyPatternService.rowcount(getKey(), StopPointInJourneyPattern.class, search);
					
					if (rowcount>0)			
					{
						errors.add("El patrón de viaje'"+scheduledstoppointId+"' está siendo usado en la relación con planificación del vehículo");
					}
