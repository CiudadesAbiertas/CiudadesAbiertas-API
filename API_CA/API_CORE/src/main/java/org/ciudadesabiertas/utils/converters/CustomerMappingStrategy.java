 ...
 * @param <T>
 */
public class CustomerMappingStrategy<T> extends ColumnPositionMappingStrategy<T> {
	...
	public String[] generateHeader(T bean) throws CsvRequiredFieldEmptyException {
		...
		//String[] header = new String[totalFieldNum];
    ...
    public String[] transmuteBean(T bean) throws CsvDataTypeMismatchException, CsvRequiredFieldEmptyException {
        ...
        int numColumns = getAnnotatedFields(bean) +1;
    ...
    private List<String> writeWithReflection(T bean, int numColumns)
        ...
        for(int i = 0; i < numColumns;) {
            ...
            if(fields.length == 0) {
                ...
                //JC: cambio para que se lea el formato y lo aplique         
            	CsvBindByName annotation = firstBeanField.getField().getAnnotation(CsvBindByName.class);
                String format=annotation.format();            	
                // Multiple values. Write the first.
                //Controlamos las comillas internas " -> '
                ...
                if (format.equals("")||(field_aux.equals("")))
                {
                	contents.add(StringUtils.defaultString(field_aux));
                }else {                	
                	contents.add(StringUtils.defaultString(String.format(format,field_aux)));
                }
