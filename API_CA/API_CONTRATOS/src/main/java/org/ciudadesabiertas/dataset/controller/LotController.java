...
import org.ciudadesabiertas.dataset.utils.LotResult;
...
@RestController
@Api(value="Lote de Contratación",description = "Conjunto de operaciones relacionadas con el conjunto de datos lotes de contratación en Contratos"+SwaggerConstants.VOCABULARIO_A_HREF+ContratosConstants.lotVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Contratos - Lote"})
public class LotController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/contract/lot";
	
	public static final String SEARCH_DISTINCT = LIST+"/distinct";
	
	public static final String RECORD = "/contract/lot/{id}";
	
	public static final String TRANSFORM = "/contract/lot/transform";
	
	public static final String ADD = "/contract/lot";
	public static final String UPDATE = "/contract/lot/{id}";
	public static final String DELETE = "/contract/lot/{id}";
	
	public static final String SEARCHGROUP = LIST +"/groupBy";
	
	public static final String MODEL_VIEW_LIST = "contratos/lotList";
	public static final String MODEL_VIEW_ID = "contratos/lotId";
	
	private static List<RequestType> listRequestType = new ArrayList<RequestType>();
	
	private static String nameController = LotController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("Contract_Lot_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("Contract_Lot_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("Contract_Lot_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("Contract_Lot_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("Contract_Lot_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("Contract_Lot_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(Lot.class);

	private static final Logger log = LoggerFactory.getLogger(LotController.class);
		

	@Autowired
	protected DatasetService<Lot> service;
	...
	@Autowired
	protected DatasetService<LotRelItem> lotRelItemService;
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO_AGRUPADA,  response=LotResult.class),
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return groupBySearch(request, groupBySearch, page, pageSize,getKey(),NO_HAY_SRID, SEARCHGROUP, new Lot(), new ObjectResult(),  service);
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new Lot(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,LotSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=LotResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, LotSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<Lot>, EntityManager> visitor = new JpaCriteriaQueryVisitor<Lot>();
		
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new Lot(), new LotResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, LotSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=LotResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getId(), obj.getHasSupplier(), obj.getTenderId(), Constants.BASIC_OPERATION_ADD);		
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=LotResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_TEXT+SwaggerConstants.PARAM_ID_CONTRATOS_LOT) 
			...
			@RequestBody Lot obj)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getId(), obj.getHasSupplier(), obj.getTenderId(), Constants.BASIC_OPERATION_UPDATE);	
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=LotResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		List<String> erroresFK = checkClavesExternas(id, null, null, Constants.BASIC_OPERATION_DELETE);	
				
		return delete(id, new Lot(), nameController, DELETE, service,getKey(),erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=LotResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_CONTRATOS_LOT) String id)
	{
		...
		return record(request, id, new Lot(),new LotResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_CONTRATOS_LOT) String id)
	{
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_CONTRATO_PROCESS_TEXT) @RequestBody Lot obj) {
	...
	private List<String> checkClavesExternas(String lotId, String awardId, String tenderId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_ADD)||(operation.equals(Constants.BASIC_OPERATION_UPDATE)))
			{
				...
				if (tenderId!=null)
				{
					...
					tenderSearch.setId(tenderId);
					long rowcount = tenderService.rowcount(getKey(), Tender.class, tenderSearch);
					if (rowcount==0)			
					{
						errors.add("The tender '"+tenderId+"' does not exist");		
					}
				}
			}			
			else if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				if (lotId!=null)
				{
					LotRelItemSearch search=new LotRelItemSearch();
					search.setLot(lotId);
					long rowcount = lotRelItemService.rowcount(getKey(), LotRelItem.class, search);
					if (rowcount>0)			
					{
						errors.add("The lot '"+lotId+"' is used in "+ rowcount+" relation of lot & item");
					}
