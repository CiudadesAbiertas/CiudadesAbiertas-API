...
import org.ciudadesabiertas.dataset.utils.AwardResult;
...
import org.ciudadesabiertas.dataset.utils.OrganizationSearch;
...
@RestController
@Api(value="Adjudicación de Contratación",description = "Conjunto de operaciones relacionadas con el conjunto de datos Organización de contratación en Contratos"+SwaggerConstants.VOCABULARIO_A_HREF+ContratosConstants.awardVocabURL+SwaggerConstants.VOCABULARIO_A_HREF_END, tags= {"Contratos - Adjudicación"})
public class AwardController extends GenericController implements CiudadesAbiertasController 
{
	public static final String LIST = "/contract/award";
	
	public static final String SEARCH_DISTINCT = LIST+"/distinct";
	
	public static final String RECORD = "/contract/award/{id}";
	
	public static final String TRANSFORM = "/contract/award/transform";
	
	public static final String ADD = "/contract/award";
	public static final String UPDATE = "/contract/award/{id}";
	public static final String DELETE = "/contract/award/{id}";
	
	public static final String MODEL_VIEW_LIST = "contratos/awardList";
	public static final String MODEL_VIEW_ID = "contratos/awardId";
	...
	private static String nameController = AwardController.class.getName();
	
	//Carga por defecto de las peticiones
	static {
		listRequestType.add(new RequestType("Contract_Award_LIST", LIST, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("Contract_Award_RECORD", RECORD, HttpMethod.GET,Constants.NO_AUTH));
		listRequestType.add(new RequestType("Contract_Award_TRANSFORM", TRANSFORM, HttpMethod.POST,Constants.NO_AUTH));
		
		listRequestType.add(new RequestType("Contract_Award_ADD", ADD, HttpMethod.POST,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("Contract_Award_UPDATE", UPDATE, HttpMethod.PUT,Constants.BASIC_AUTH));
		listRequestType.add(new RequestType("Contract_Award_DELETE", DELETE, HttpMethod.DELETE,Constants.BASIC_AUTH));
		
	}
	
	public static List<String> availableFields=Util.extractPropertiesFromBean(Award.class);

	private static final Logger log = LoggerFactory.getLogger(AwardController.class);
		

	@Autowired
	protected DatasetService<Award> service;
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO_AGRUPADA,  response=AwardResult.class),
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return groupBySearch(request, groupBySearch, page, pageSize,getKey(),NO_HAY_SRID, SEARCHGROUP, new Award(), new ObjectResult(),  service);
					...
					@ApiParam(value=SwaggerConstants.PARAM_PAGESIZE) String pageSize)
	{
		...
		return distinctSearch(request, search, availableFields, page, pageSize,getKey(),NO_HAY_SRID, SEARCH_DISTINCT, new Award(), new ObjectResult(),  service);
	...
	public ModelAndView listHTML(
			ModelAndView mv, HttpServletRequest request,AwardSearch search, 
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_BUSQUEDA_O_LISTADO,  response=AwardResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	   })
	@RequestMapping(value= {LIST,  VERSION_1+LIST}, method = {RequestMethod.GET})	
	public @ResponseBody ResponseEntity<?> list(HttpServletRequest request, AwardSearch search, 
			...
			@RequestHeader HttpHeaders headersRequest)
	{
		...
		RSQLVisitor<CriteriaQuery<Award>, EntityManager> visitor = new JpaCriteriaQueryVisitor<Award>();
		return list(request, search, fields, rsqlQ, page, pageSize, sort, NO_HAY_SRID, LIST,new Award(), new AwardResult(), 
	...
	public @ResponseBody ResponseEntity<?> listHead(HttpServletRequest request, AwardSearch search, 
			@RequestParam(value = Constants.FIELDS, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_FIELDS) String fields, 
			@RequestParam(value = Constants.RSQL_Q, defaultValue = "", required = false) 
				@ApiParam(value=SwaggerConstants.PARAM_Q) String rsqlQ, 
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_INSERCION,  response=AwardResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getId(), obj.getIsSupplierFor(), Constants.BASIC_OPERATION_ADD);
	...
	@ApiResponses({
	            @ApiResponse(code = 201, message = SwaggerConstants.RESULTADO_DE_MODIFICACION,  response=AwardResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> update(
			@ApiParam(required = true, name = Constants.IDENTIFICADOR, value = SwaggerConstants.PARAM_ID_TEXT+SwaggerConstants.PARAM_ID_CONTRATOS_AWARD) 
			...
			@RequestBody Award obj)
	{
		...
		List<String> erroresFK = checkClavesExternas(obj.getId(), obj.getIsSupplierFor(), Constants.BASIC_OPERATION_UPDATE);
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_DELETE,  response=AwardResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
			...
			@PathVariable(Constants.IDENTIFICADOR) String id)
	{
		...
		List<String> erroresFK = checkClavesExternas(id, null, Constants.BASIC_OPERATION_DELETE);
		
		return delete(id, new Award(), nameController, DELETE, service,getKey(), erroresFK);
	}
	...
	@ApiResponses({
	            @ApiResponse(code = 200, message = SwaggerConstants.RESULTADO_DE_FICHA,  response=AwardResult.class),
	            @ApiResponse(code = 400, message = SwaggerConstants.PETICION_INCORRECTA,  response=ResultError.class),
	            @ApiResponse(code = 401, message = SwaggerConstants.NO_AUTORIZADO,  response=ResultError.class),
	            @ApiResponse(code = 409, message = SwaggerConstants.EL_RECURSO_YA_EXISTE,  response=ResultError.class),
	            @ApiResponse(code = 500, message = SwaggerConstants.ERROR_INTERNO,  response=ResultError.class)
	...
	public @ResponseBody ResponseEntity<?> record(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_CONTRATOS_AWARD) String id)
	{
		...
		return record(request, id, new Award(),new AwardResult(), NO_HAY_SRID, nameController, RECORD, service,getKey());
	...
	public @ResponseBody ResponseEntity<?> recordHead(HttpServletRequest request, @PathVariable @ApiParam(required = true, value=SwaggerConstants.PARAM_ID+SwaggerConstants.PARAM_ID_CONTRATOS_AWARD) String id)
	{
	...
	public @ResponseBody ResponseEntity<?> transform(
			@ApiParam(required = true, name = Constants.OBJETO, value = SwaggerConstants.PARAM_CONTRATO_PROCESS_TEXT) @RequestBody Award obj) {
	...
	private List<String> checkClavesExternas(String awardId, String organizationId, String operation) {
		...
		{	
			if (operation.equals(Constants.BASIC_OPERATION_ADD)||(operation.equals(Constants.BASIC_OPERATION_UPDATE)))
			{
				if (organizationId!=null)
				{
					...
					long rowcount = organizationService.rowcount(getKey(), Organization.class, orgSearch);
					if (rowcount==0)			
					{
						errors.add("The organization '"+organizationId+"' does not exist");		
					}
				}
			}			
			else if (operation.equals(Constants.BASIC_OPERATION_DELETE))
			{
				if (awardId!=null)
				{
					...
					lotSearch.setHasSupplier(awardId);
					long rowcount = lotService.rowcount(getKey(), Lot.class, lotSearch);
					if (rowcount>0)			
					{
						errors.add("The award '"+awardId+"' is used in "+ rowcount+" Lots");
					}
					
					TenderSearch tenderSearch=new TenderSearch();
					tenderSearch.setHasSupplier(awardId);
					rowcount = tenderService.rowcount(getKey(), Tender.class, tenderSearch);
					if (rowcount>0)			
					{
						errors.add("The award '"+awardId+"' is used in "+ rowcount+" Tenders");
					}
