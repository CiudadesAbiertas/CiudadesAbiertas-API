...
@Entity
@ApiModel
@Table(name = "contratos_tender_rel_item")
@JsonInclude(JsonInclude.Include.NON_EMPTY)
@JsonPropertyOrder(alphabetic = false)
@JsonIgnoreProperties({ Constants.IKEY })
@JacksonXmlRootElement(localName = Constants.RECORD)
@PathId(value = "/contract/item")
public class TenderRelItem implements java.io.Serializable, RDFModel {

	@JsonIgnore
	private static final long serialVersionUID = 376859484313928528L;
	...
	@Rdf(contexto = Context.OCDS, propiedad = "isProcuredInTender")
	@RdfExternalURI(inicioURI = "/contract/tender/", finURI = "tender", urifyLevel = 1)
	private String item;

	public TenderRelItem() {
	}

	public TenderRelItem(TenderRelItem copia) {
		super();
		this.ikey = copia.ikey;
		this.id = copia.id;
		this.tender = copia.tender;
		this.item = copia.item;
	}

	public TenderRelItem(TenderRelItem copia, List<String> attributesToSet) {
		super();
		...
		if (attributesToSet.contains("tender")) {
			this.tender = copia.tender;
		}
	}
	...
	public List<String> validate() {
		...
		if (!Util.validValue(this.getTender())) {
			result.add("Tender is not valid [Title:" + this.getTender() + "]");
		}
	...
	public String toString() {
		return "TenderRelItem [ikey=" + ikey + ", id=" + id + ", tender=" + tender + ", item=" + item + "]";
	}
