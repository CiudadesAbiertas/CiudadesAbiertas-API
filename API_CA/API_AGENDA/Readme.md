...
# API CIUDADES ABIERTAS - MÓDULO AGENDA

Esta es la documentación asociada al módulo de Agenda.
...
La API de Ciudades Abiertas permite realizar configuraciones específicas asociadas a cada uno de los módulos que se integran.

Para realizar esto lo único que se necesita es crear un fichero **agenda.properties** e incluirlo en la carpeta de recursos del proyecto WEB (API_WEB/src/main/resources/) donde se distribuyen el resto de ficheros de configuración.

Esto permite que el módulo de **agenda** utilice la información que contenga este fichero cuando se ejecutan peticiones sobre dicho módulo.

A continuación se detallan los parámetros que se pueden configurar en este fichero **agenda.properties**.
...
- **AGENDA_LIST**: Listado de eventos de la agenda
- **AGENDA_RECORD**: Ficha de un evento de la agenda
- **AGENDA_ADD**:  Alta de un evento en la agenda
- **AGENDA_UPDATE**: Modificación de un evento en la agenda
- **AGENDA_DELETE**: Baja de un evento en la agenda
- **AGENDA_TRANSFORM**: Transformador de un evento externo (a través de JSON) a cualquiera de los formatos soportados por la API

Y los parámetros configurados quedarían de la siguiente manera en el fichero (**agenda.properties**) Como Ejemplo:
- **peticiones.identificadas.public_auth=AGENDA_ADD,AGENDA_RECORD,AGENDA_LIST**
- **peticiones.identificadas.basic_auth= AGENDA_UPDATE,AGENDA_TRANSFORM**
- **peticiones.identificadas.admin_auth= AGENDA_DELETE**
